                ...
                ['full_opt', 'min_opt'].each { opt_level ->
                    ...
                    def newJob = job(Utilities.getFullJobName(project, jobName, isPR)) {
                        ...
                        steps {
                            ...
                            def runXUnitPerfCommonArgs = "-arch ${arch} -configuration ${configuration} -os Windows_NT -generateBenchviewData \"%WORKSPACE%\\Microsoft.Benchview.JSONFormat\\tools\" ${uploadString} -runtype ${runType} ${testEnv} -optLevel ${opt_level} -jitName ${jit} -outputdir \"%WORKSPACE%\\bin\\sandbox_logs\" -stabilityPrefix \"START \\\"CORECLR_PERF_RUN\\\" /B /WAIT /HIGH /AFFINITY 0x2\""
                ...
                ['full_opt'].each { opt_level ->
                    ...
                    def newJob = job(Utilities.getFullJobName(project, "perf_illink_${os}_${arch}_${opt_level}_${jit}", isPR)) {
                        ...
                        steps {
                            ...
                            def runXUnitPerfCommonArgs = "-arch ${arch} -configuration ${configuration} -os Windows_NT -generateBenchviewData \"%WORKSPACE%\\Microsoft.Benchview.JSONFormat\\tools\" ${uploadString} -runtype ${runType} ${testEnv} -optLevel ${opt_level} -jitName ${jit} -outputdir \"%WORKSPACE%\\bin\\sandbox_logs\" -scenarioTest"
