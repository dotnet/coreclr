...
#endif
{
    ...
    public class EventCounter : IDisposable
    {
        ...
        public EventCounter(string name, EventSource eventSource, Func<float> getMetricValueFunc)
        {
            if (name == null)
            {
                throw new ArgumentNullException(nameof(name));
            ...
            _min = float.PositiveInfinity;
            _max = float.NegativeInfinity;
            _getMetricValueFunc = getMetricValueFunc;
        }
        ...
        {
            Enqueue(this._getMetricValueFunc());
        }
        ...
        private Func<float> _getMetricValueFunc;
    ...
    internal class EventCounterPayload : IEnumerable<KeyValuePair<string, object>>
    {
        ...
        {
            return "EventCounter '" + Name + "' Count " + Count + " Mean " + Mean.ToString("n3");
        }
    ...
    internal class EventCounterGroup
    {
        ...
        private void OnEventSourceCommand(object sender, EventCommandEventArgs e)
        {
            if (e.Command == EventCommand.Enable || e.Command == EventCommand.Update)
            {
                ...
                else // TODO: maybe we don't need this once we are able to read commands from named pipes
                {
                    lock (this)
                    {
                        EnableTimer(1);
