...
namespace System.Runtime.InteropServices
{
    public static partial class Marshal
    {
        ...
        internal static unsafe int ConvertStringToAnsi(string s, byte* buffer, int wideCharLen, bool bestFit = false, bool throwOnUnmappableChar = false)
        {
            Debug.Assert(wideCharLen >= (s.Length + 1) * SystemMaxDBCSCharSize, "Insufficient buffer length passed to ConvertStringToAnsi");

            int nb;

            uint flgs = bestFit ? 0 : Interop.Kernel32.WC_NO_BEST_FIT_CHARS;
