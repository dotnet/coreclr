    ...
    public class SemaphoreSlim : IDisposable
    {
        ...
        public bool Wait(int millisecondsTimeout, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!(m_waitHandle is null) && m_currentCount == 0)
                    {
                        m_waitHandle.Reset();
                    }
                }
            ...
            return (!(asyncWaitTask is null)) ? asyncWaitTask.GetAwaiter().GetResult() : waitSuccessful;
        }
        ...
        public Task<bool> WaitAsync(int millisecondsTimeout, CancellationToken cancellationToken)
        {
            ...
            lock (m_lockObj)
            {
                ...
                {
                    --m_currentCount;
                    if (!(m_waitHandle is null) && m_currentCount == 0) m_waitHandle.Reset();
                    return s_trueTask;
                }
        ...
        private bool RemoveAsyncWaiter(TaskNode task)
        {
            ...
            bool wasInList = m_asyncHead == task || !(task.Prev is null);

            // Remove it from the linked list
            if (!(task.Next is null)) task.Next.Prev = task.Prev;
            if (!(task.Prev is null)) task.Prev.Next = task.Next;
        ...
        public int Release(int releaseCount)
        {
            ...
            lock (m_lockObj)
            {
                ...
                if (!(m_asyncHead is null))
                {
                    ...
                    while (maxAsyncToRelease > 0 && !(m_asyncHead is null))
                    {
                        --currentCount;
                        --maxAsyncToRelease;
                ...
                if (!(m_waitHandle is null) && returnCount == 0 && currentCount > 0)
                {
                    m_waitHandle.Set();
                }
            }
