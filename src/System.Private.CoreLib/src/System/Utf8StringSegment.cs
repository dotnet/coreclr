...
namespace System
{
    ...
    public readonly struct Utf8StringSegment : IEquatable<Utf8StringSegment>
    {
        ...
        public Utf8StringSegment(Utf8String value, int offset, int count)
        {
            if (value != null)
            {
                if (((uint)offset > (uint)value.Length) || ((uint)(offset + count) > (uint)value.Length))
                {
        ...
        {
            if (((uint)startIndex > (uint)_count) || ((uint)(startIndex + length) > (uint)_count))
            {
                // TODO: better exception
                throw new Exception("Argument out of range.");
            }
        ...
        public Utf8StringSegment Trim() => TrimHelper(Utf8String.TrimType.Both);

        public Utf8StringSegment TrimEnd() => TrimHelper(Utf8String.TrimType.Tail);

        private Utf8StringSegment TrimHelper(Utf8String.TrimType trimType)
        {
            Utf8StringSegment retVal = this;

            if (trimType.HasFlag(Utf8String.TrimType.Head))
            {
                retVal = retVal.Substring(Utf8Utility.GetIndexOfFirstNonWhiteSpaceChar(retVal.AsSpan()));
            }

            if (trimType.HasFlag(Utf8String.TrimType.Tail))
            {
        ...
        public Utf8StringSegment TrimStart() => TrimHelper(Utf8String.TrimType.Head);
    }
}
