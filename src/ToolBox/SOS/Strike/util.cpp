...
HRESULT SymbolReader::LoadSymbols(IMetaDataImport* pMD, ICorDebugModule* pModule)
{
    ...
    if (isDynamic)
    {
        // Dynamic and in memory assemblies are a special case which we will ignore for now now
...
#ifndef FEATURE_PAL
    if (SUCCEEDED(LoadSymbolsForWindows(pMD, peAddress, moduleName, isInMemory)))
    {
        return S_OK;
    }
#endif // FEATURE_PAL
    return LoadSymbolsForPortablePDB(isInMemory, moduleName, peAddress, peSize, 0, 0);
}

HRESULT SymbolReader::LoadSymbols(IMetaDataImport* pMD, IXCLRDataModule* pModule)
{
    ...
    hr = LoadSymbolsForWindows(pMD, moduleData.LoadedPEAddress, pModuleName, moduleData.bIsInMemory);
    if (SUCCEEDED(hr))
    {
        return hr;
    }
...
HRESULT SymbolReader::LoadSymbolsForWindows(IMetaDataImport* pMD, ULONG64 peAddress, __in_z WCHAR* pModuleName, BOOL isInMemory)
{
    HRESULT Status = S_OK;
...
HRESULT SymbolReader::LoadSymbolsForPortablePDB(BOOL isInMemory, __in_z WCHAR* pModuleName, ULONG64 peAddress, 
    ULONG64 peSize, ULONG64 inMemoryPdbAddress, ULONG64 inMemoryPdbSize)
{
    ...
    m_symbolReaderHandle = loadSymbolsForModuleDelegate(szModuleName, peAddress, (int)peSize, inMemoryPdbAddress, (int)inMemoryPdbSize, ReadMemoryForSymbols);
...
HRESULT SymbolReader::GetLineByILOffset(mdMethodDef methodToken, ULONG64 ilOffset,
    ___out ULONG *pLinenum, __out_ecount(cbFileName) WCHAR* pwszFileName, ___in ULONG cchFileName)
{
    HRESULT Status = S_OK;
...
HRESULT SymbolReader::GetNamedLocalVariable(ISymUnmanagedScope * pScope, ICorDebugILFrame * pILFrame, mdMethodDef methodToken, 
    ULONG localIndex, __inout_ecount(paramNameLen) WCHAR* paramName, ULONG paramNameLen, ICorDebugValue** ppValue)
{
    HRESULT Status = S_OK;
...
HRESULT SymbolReader::GetNamedLocalVariable(ICorDebugFrame * pFrame, ULONG localIndex, 
    __inout_ecount(paramNameLen) WCHAR* paramName, ULONG paramNameLen, ICorDebugValue** ppValue)
{
    HRESULT Status = S_OK;
