    ...
    *szptr++ = '"';
    ...
    do
    {
        ...
        if (tally)
        {
            ...
            uMaxLen = uMaxLen - (ULONG32)strlen(szPrefix) + Lwt;
        tally++;
        for (i = 0; i < l; i++, szptr++, p++)
        {
            ...
            case '"':
                ...
                *szptr = '"';
                break;
            case '{':
                szptr += sprintf_s(szptr, SZSTRING_REMAINING_SIZE(szptr), SCOPE());
                szptr--;
                break;
            case '}':
                szptr += sprintf_s(szptr, SZSTRING_REMAINING_SIZE(szptr), UNSCOPE());
                szptr--;
                break;
            case '<':
...
static int __cdecl ehInfoCmp(const void *op1, const void *op2)
{
    ...
    int d;
    d = p1->GetTryOffset() - p2->GetTryOffset();
    if (d)
        return d;
    d = p1->GetTryLength() - p2->GetTryLength();
    if (d)
        return d;
    d = p1->GetHandlerOffset() - p2->GetHandlerOffset();
    if (d)
        return d;
    d = p1->GetHandlerLength() - p2->GetHandlerLength();
    if (d)
        return d;
    return 0;
}
...
static int __cdecl cmpLineCode(const void *p1, const void *p2)
{
    ...
    if (ret == 0)
    {
        ...
        if (ret == 0)
            ret = (((LineCodeDescr *)p1)->Column - ((LineCodeDescr *)p2)->Column);
    }
    return ret;
}
...
static int __cdecl cmpLexScope(const void *p1, const void *p2)
{
    ...
    int d;
    if ((d = pls1->dwStart - pls2->dwStart))
        return d;
...
char *DumpDataPtr(__inout __nullterminated char *buffer, DWORD ptr, DWORD size)
{
    ...
    for (i = 0; i < dwNumberOfSections; i++, pSecHdr++)
    {
        ...
        {
            if (ptr + size > VAL32(pSecHdr->VirtualAddress) + VAL32(pSecHdr->Misc.VirtualSize))
                i = dwNumberOfSections;
            break;
        }
    }
    ...
    { // yes, the pointer points to real data
        ...
        else if ((*g_pPtrSize)[j] < size)
            (*g_pPtrSize)[j] = size;
        const char *szTls = "D_";
        if (strcmp((const char *)(pSecHdr->Name), ".tls") == 0)
            szTls = "T_";
        else if (strcmp((const char *)(pSecHdr->Name), ".text") == 0)
            szTls = "I_";
                ...
                for (pch = pszTailSig, i = 0, j = 0, k = 0; pch < pComma; pch++)
                {
                    ...
                    else if (j == 0)
                    {
                        if (*pch == '[')
                            ...
                            i--;
                        else if (strncmp(pch, LTN(), strlen(LTN())) == 0)
                            k++;
                        else if (strncmp(pch, GTN(), strlen(GTN())) == 0)
                            k--;
                    }
                }
                pComma++;
               ...
               ULONG ulLocals)
{
    ...
    if (FAILED(pIScope->GetLocalCount(&ulVars)))
        return;

    ULONG32 ulNameLen;
#ifdef SHOW_LEXICAL_SCOPES
    for (unsigned jj = 0; jj < ulLocals; jj++)
        pLV[jj].attr = 0xFFFFFFFF;
#endif
    if (ulVars)
    {
...
BOOL SourceLinesHelper(void *GUICookie, LineCodeDescr *pLCD, __out_ecount(nSize) WCHAR *pFileName, UINT nSize)
{
    ...
    PAL_TRY(Param *, pParam, &param)
    {
        ...
        if (pParam->pLCD->FileToken != ulWasFileToken)
        {
            ...
            if (memcmp(&guidLang, &guidWasLang, sizeof(GUID)) || memcmp(&guidLangVendor, &guidWasLangVendor, sizeof(GUID)) || memcmp(&guidDoc, &guidWasDoc, sizeof(GUID)))
            {
...
BOOL Disassemble(IMDInternalImport *pImport, BYTE *ILHeader, void *GUICookie, mdToken FuncToken, ParamDescriptor *pszArgname, ULONG ulArgs)
{
    ...
    if (g_pSymReader)
    {
        ...
        if (method.GetLocalVarSigTok())
        {
            ...
            if (pSymMethod)
                pSymMethod->GetRootScope(&pRootScope);
            else
                pRootScope = NULL;
...
#pragma warning(default : 4640)
    ...
    while (PC < method.GetCodeSize())
    {
            ...
            while (PC == pLCD->PC)
            {
                ...
                if (fInsertSourceLines)
                {
                    if (bIsNewFile)
                    {
                        ...
                        sprintf_s(szString, SZSTRING_SIZE, (*pszFN == '\'' ? "%s%s %d,%d : %d,%d %s" : "%s%s %d,%d : %d,%d '%s'"),
        ...
        if (fTryInCode)
        {
            ...
            for (ii = g_ehCount; ii > 0; ii--)
            {
                ...
                else
                {
                    ...
                    if (theTryEnd > theEnd)
                        theEnd = theTryEnd; // try block after the handler
        ...
        if (g_fShowBytes)
        {
            ...
            for (; i < 2; i++)
                szptr += sprintf_s(szptr, SZSTRING_REMAINING_SIZE(szptr), "  "); // 2 is max.opcode length
...
#define PadTheString                                                                              \
    {                                                                                             \
        if (Len < 16)                                                                             \
            szptr += sprintf_s(szptr, SZSTRING_REMAINING_SIZE(szptr), "%-*s", (16 - Len), "");    \
        ...
        case ShortInlineVar:
        {
            ...
            case CEE_STARG_S:
                ...
                else
                {
                    if (pszArgname)
                    {
                        ...
                        szptr += sprintf_s(szptr, SZSTRING_REMAINING_SIZE(szptr), "%-10s A_%d", pszInstrName, ch);
                }
                break;
            ...
            case CEE_STLOC_S:
                if (pszLVname)
                {
                    ...
                    szptr += sprintf_s(szptr, SZSTRING_REMAINING_SIZE(szptr), "%-10s V_%d", pszInstrName, ch);
                break;
        ...
        case InlineVar:
        {
            ...
            case CEE_STARG:
                ...
                else
                {
                    if (pszArgname)
                    {
                        ...
                        szptr += sprintf_s(szptr, SZSTRING_REMAINING_SIZE(szptr), "%-10s A_%d", pszInstrName, v);
                }
                break;
            ...
            case CEE_STLOC:
                if (pszLVname)
                {
                    ...
                    szptr += sprintf_s(szptr, SZSTRING_REMAINING_SIZE(szptr), "%-10s V_%d", pszInstrName, v);
                break;
    ...
    if (pszLVname)
    {
        for (ULONG i = 0; i < ulVars; i++)
            SDELETE(pszLVname[i].name);
        VDELETE(pszLVname);
    }
    //  dumpEHInfo(pImport, GUICookie);
        ...
        while ((pComma = strchr(pComma, ',')) != NULL)
        {
            ...
            if ((i == 0) && (j == 0) && (k <= 1) && (l == 0)) // no brackets or all opened/closed
            {
                ...
                *pComma = chAfterComma;
                ...
                for (i = (ULONG32)strlen(g_szAsmCodeIndent); i < indent; i++)
                    pszTailSig[i] = ' ';
                strcpy_s(&pszTailSig[indent], strlen(pComma) + 1, pComma);
                pComma = pszTailSig;
            }
                           ...
                           CQuickBytes *pqbMemberSig)
{
    ...
    *curPos = 0;
    ...
    if (TypeFromToken(cr) == mdtMethodDef) //get the parent's parent
    {
        ...
        mdTypeRef cr1;
        if (FAILED(pImport->GetParentToken(cr, &cr1)))
            cr1 = mdTypeRefNil;
        cr = cr1;
    }
...
void PrettyPrintTokenWithSplit(__inout __nullterminated char *szString, mdToken tk, IMDInternalImport *pImport, void *GUICookie)
{
    ...
    {
    case mdtMethodDef:
        SigToken = TSA_MD->ptsArray;
        break;
    case mdtMethodSpec:
        SigToken = TSA_MS->ptsArray;
        break;
    case mdtMemberRef:
        SigToken = TSA_ME->ptsArray;
        break;
    default:
        return; // other token types aren't served
    }
...
#endif
    {
        ...
        case mdtMethodSpec:
        {
            ...
            pszTailSig = (TypeFromToken(meth) == mdtMethodDef) ? PrettyPrintMethodDef(szString, meth, pSig, cSig, pImport, GUICookie, &indent, &qbMemberSig) : PrettyPrintMemberRef(szString, meth, pSig, cSig, pImport, GUICookie, &indent, &qbMemberSig);
...
void PrettyPrintToken(__inout __nullterminated char *szString, mdToken tk, IMDInternalImport *pImport, void *GUICookie, mdToken FuncToken)
{
    ...
    {
        if ((*g_typedefs)[ix].tkTypeSpec == tk)
            break;
    }
    if (ix < g_NumTypedefs)
    {
    ...
    {
    case mdtTypeDef:
        SigToken = TSA_TD->ptsArray;
        break;
    case mdtTypeRef:
        SigToken = TSA_TR->ptsArray;
        break;
    case mdtTypeSpec:
        SigToken = TSA_TS->ptsArray;
        break;
    case mdtAssemblyRef:
        SigToken = TSA_AR->ptsArray;
        break;
    case mdtModuleRef:
        SigToken = TSA_MR->ptsArray;
        break;
    case mdtFieldDef:
        SigToken = TSA_FD->ptsArray;
        break;
    default:
        SigToken = NULL;
        break;
...
{
    return (strchr(szf, '#') != NULL) || (strstr(szf, "nan") != NULL) || (strstr(szf, "NAN") != NULL) || (strstr(szf, "inf") != NULL) || (strstr(szf, "INF") != NULL);
}
