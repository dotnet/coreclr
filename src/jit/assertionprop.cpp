                                            ...
                                            AssertionDsc*    assertion)
{
    ...
    if (op1->gtOper == GT_ARR_BOUNDS_CHECK)
    {
        if (assertionKind == OAK_NO_THROW)
        {
            ...
            assertion->op1.bnd.vnIdx     = vnStore->VNConservativeNormVal(arrBndsChk->gtIndex->gtVNPair);
            assertion->op1.bnd.vnLen     = vnStore->VNConservativeNormVal(arrBndsChk->gtArrLen->gtVNPair);
            goto DONE_ASSERTION;
        }
    }
    ...
    if (op2 == nullptr)
    {
        ...
        if (lclVar->TypeGet() != TYP_REF)
        {
            ...
            vn = vnStore->VNConservativeNormVal(op1->gtVNPair);
            VNFuncApp funcAttr;
    ...
    else if (op1->gtOper == GT_LCL_VAR)
    {
        ...
        if (haveArgs)
        {
            ...
            assertion->op1.lcl.lclNum   = lclNum;
            assertion->op1.vn           = vnStore->VNConservativeNormVal(op1->gtVNPair);
            assertion->op1.lcl.ssaNum   = op1->AsLclVarCommon()->GetSsaNum();
            assertion->op2.u1.iconVal   = op2->gtIntCon.gtIconVal;
            assertion->op2.vn           = vnStore->VNConservativeNormVal(op2->gtVNPair);
    ...
    else if (op1->gtOper == GT_IND)
    {
        ...
        if (op1->gtOper == GT_LCL_VAR)
        {
            ...
            if (op2->gtOper == GT_IND)
            {
                ...
                assertion->op2.vn         = vnStore->VNConservativeNormVal(op2->gtOp.gtOp1->gtVNPair);
...
bool Compiler::optIsTreeKnownIntValue(bool vnBased, GenTree* tree, ssize_t* pConstant, unsigned* pFlags)
{
    ...
    ValueNum vn = vnStore->VNConservativeNormVal(tree->gtVNPair);
...
AssertionInfo Compiler::optCreateJTrueBoundsAssertion(GenTree* tree)
{
    ...
    ValueNum op1VN   = vnStore->VNConservativeNormVal(op1->gtVNPair);
    ValueNum relopVN = vnStore->VNConservativeNormVal(relop->gtVNPair);
    ValueNum op2VN   = vnStore->VNConservativeNormVal(op2->gtVNPair);
    ...
    // Assertion: "i < bnd +/- k == 0"
    if (vnStore->IsVNCompareCheckedBoundArith(op1VN) && op2VN == vnStore->VNZeroForType(op2->TypeGet()) &&
    ...
    else if (vnStore->IsVNCompareCheckedBound(op1VN) && (op2VN == vnStore->VNZeroForType(op2->TypeGet())) &&
             (relop->gtOper == GT_EQ || relop->gtOper == GT_NE))
    {
        AssertionDsc dsc;
        dsc.assertionKind    = relop->gtOper == GT_EQ ? OAK_EQUAL : OAK_NOT_EQUAL;
    ...
    else if (vnStore->IsVNUnsignedCompareCheckedBound(relopVN, &unsignedCompareBnd))
    {
        ...
        dsc.op1.bnd.vnLen = vnStore->VNNormVal(unsignedCompareBnd.vnBound);
    ...
    else if (vnStore->IsVNConstantBound(op1VN) && (op2VN == vnStore->VNZeroForType(op2->TypeGet())) &&
             (relop->gtOper == GT_EQ || relop->gtOper == GT_NE))
    {
        AssertionDsc dsc;
        dsc.assertionKind    = relop->gtOper == GT_EQ ? OAK_EQUAL : OAK_NOT_EQUAL;
    ...
    for (AssertionIndex index = 1; index <= optAssertionCount; index++)
    {
            ...
            (curAssertion->op1.kind == O1K_LCLVAR))
        {
            ...
            bool isEqual = optLocalAssertionProp
                               ...
                               : (curAssertion->op1.vn == vnStore->VNConservativeNormVal(tree->gtVNPair));
            if (!isEqual)
            {
                continue;
            }
...
GenTree* Compiler::optVNConstantPropOnTree(BasicBlock* block, GenTree* stmt, GenTree* tree)
{
    ...
    ValueNum vnLib = vnStore->VNLiberalNormVal(tree->gtVNPair);
    ValueNum vnCns = vnStore->VNConservativeNormVal(tree->gtVNPair);
...
GenTree* Compiler::optAssertionProp_LclVar(ASSERT_VALARG_TP assertions, GenTree* tree, GenTree* stmt)
{
    ...
    while (iter.NextElem(&index))
    {
                 ...
                 tree->gtType == lvaTable[tree->gtLclVarCommon.GetLclNum()].lvType)
        {
            ...
            else if (curAssertion->op1.vn == vnStore->VNConservativeNormVal(tree->gtVNPair))
            {
#if FEATURE_ANYCSE
...
AssertionIndex Compiler::optGlobalAssertionIsEqualOrNotEqual(ASSERT_VALARG_TP assertions, GenTree* op1, GenTree* op2)
{
    ...
    while (iter.NextElem(&index))
    {
        ...
        if (curAssertion->op1.vn == vnStore->VNConservativeNormVal(op1->gtVNPair) &&
            curAssertion->op2.vn == vnStore->VNConservativeNormVal(op2->gtVNPair))
        {
            return assertionIndex;
        }
    }
...
GenTree* Compiler::optAssertionPropGlobal_RelOp(ASSERT_VALARG_TP assertions, GenTree* tree, GenTree* stmt)
{
    ...
    ValueNum vnCns = vnStore->VNConservativeNormVal(op2->gtVNPair);
    if (vnStore->IsVNConstant(vnCns))
    {
#ifdef DEBUG
        if (verbose)
...
GenTree* Compiler::optAssertionProp_BndsChk(ASSERT_VALARG_TP assertions, GenTree* tree, GenTree* stmt)
{
    ...
    while (iter.NextElem(&index))
    {
        ...
        if (curAssertion->op1.bnd.vnLen == vnStore->VNConservativeNormVal(arrBndsChk->gtArrLen->gtVNPair))
        {
            ValueNum vnCurIdx = vnStore->VNConservativeNormVal(arrBndsChk->gtIndex->gtVNPair);
    ...
    if ((oldTree->gtFlags & GTF_SIDE_EFFECT) != 0)
    {
        ...
        if (oldTree == newTree)
        {
            ...
            assert(!gtNodeHasSideEffects(oldTree, GTF_EXCEPT) ||
                   vnStore->IsVNConstant(vnStore->VNConservativeNormVal(oldTree->gtVNPair)));
...
GenTree* Compiler::optVNConstantPropOnJTrue(BasicBlock* block, GenTree* stmt, GenTree* test)
{
    ...
    if (!vnStore->IsVNConstant(vnStore->VNConservativeNormVal(relop->gtVNPair)))
    {
        return nullptr;
    }
    ...
    ValueNum vnCns       = vnStore->VNConservativeNormVal(relop->gtVNPair);
