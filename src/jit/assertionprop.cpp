                                            ...
                                            AssertionDsc*    assertion)
{
    ...
    if (op1->gtOper == GT_ARR_BOUNDS_CHECK)
    {
        if (assertionKind == OAK_NO_THROW)
        {
            ...
            assertion->op1.bnd.vnIdx     = vnStore->VNNormVal(arrBndsChk->gtIndex->gtVNPair.GetConservative());
            assertion->op1.bnd.vnLen     = vnStore->VNNormVal(arrBndsChk->gtArrLen->gtVNPair.GetConservative());
            goto DONE_ASSERTION;
        }
    }
    ...
    if (op2 == nullptr)
    {
        ...
        if (lclVar->TypeGet() != TYP_REF)
        {
            ...
            vn = vnStore->VNNormVal(op1->gtVNPair.GetConservative());
            VNFuncApp funcAttr;
    ...
    else if (op1->gtOper == GT_LCL_VAR)
    {
        ...
        if (haveArgs)
        {
            ...
            assertion->op1.lcl.lclNum   = lclNum;
            assertion->op1.vn           = vnStore->VNNormVal(op1->gtVNPair.GetConservative());
            assertion->op1.lcl.ssaNum   = op1->AsLclVarCommon()->GetSsaNum();
            assertion->op2.u1.iconVal   = op2->gtIntCon.gtIconVal;
            assertion->op2.vn           = vnStore->VNNormVal(op2->gtVNPair.GetConservative());
    ...
    else if (op1->gtOper == GT_IND)
    {
        ...
        if (op1->gtOper == GT_LCL_VAR)
        {
            ...
            if (op2->gtOper == GT_IND)
            {
                ...
                assertion->op2.vn         = vnStore->VNNormVal(op2->gtOp.gtOp1->gtVNPair.GetConservative());
...
bool Compiler::optIsTreeKnownIntValue(bool vnBased, GenTree* tree, ssize_t* pConstant, unsigned* pFlags)
{
    ...
    ValueNum vn = vnStore->VNNormVal(tree->gtVNPair.GetConservative());
...
AssertionInfo Compiler::optCreateJTrueBoundsAssertion(GenTree* tree)
{
    ...
    // Assertion: "i < bnd +/- k == 0"
    if (vnStore->IsVNCompareCheckedBoundArith(vn) && op2VN == vnStore->VNZeroForType(op2->TypeGet()) &&
    ...
    else if (vnStore->IsVNCompareCheckedBound(vn) && (op2VN == vnStore->VNZeroForType(op2->TypeGet())) &&
             (relop->gtOper == GT_EQ || relop->gtOper == GT_NE))
    {
        AssertionDsc dsc;
        dsc.assertionKind    = relop->gtOper == GT_EQ ? OAK_EQUAL : OAK_NOT_EQUAL;
    ...
    else if (vnStore->IsVNConstantBound(vn) && (op2VN == vnStore->VNZeroForType(op2->TypeGet())) &&
             (relop->gtOper == GT_EQ || relop->gtOper == GT_NE))
    {
        AssertionDsc dsc;
        dsc.assertionKind    = relop->gtOper == GT_EQ ? OAK_EQUAL : OAK_NOT_EQUAL;
    ...
    for (AssertionIndex index = 1; index <= optAssertionCount; index++)
    {
            ...
            (curAssertion->op1.kind == O1K_LCLVAR))
        {
            ...
            bool isEqual = optLocalAssertionProp
                               ...
                               : (curAssertion->op1.vn == vnStore->VNNormVal(tree->gtVNPair.GetConservative()));
            if (!isEqual)
            {
                continue;
            }
...
GenTree* Compiler::optVNConstantPropOnTree(BasicBlock* block, GenTree* stmt, GenTree* tree)
{
    ...
    if (!tree->OperMayThrow(this))
    {
        vnCns = vnStore->VNNormVal(vnCns);
        vnLib = vnStore->VNNormVal(vnLib);
    }
...
GenTree* Compiler::optAssertionProp_LclVar(ASSERT_VALARG_TP assertions, GenTree* tree, GenTree* stmt)
{
    ...
    while (iter.NextElem(&index))
    {
                 ...
                 tree->gtType == lvaTable[tree->gtLclVarCommon.GetLclNum()].lvType)
        {
            ...
            else if (curAssertion->op1.vn == vnStore->VNNormVal(tree->gtVNPair.GetConservative()))
            {
#if FEATURE_ANYCSE
...
AssertionIndex Compiler::optGlobalAssertionIsEqualOrNotEqual(ASSERT_VALARG_TP assertions, GenTree* op1, GenTree* op2)
{
    ...
    while (iter.NextElem(&index))
    {
        ...
        if (curAssertion->op1.vn == vnStore->VNNormVal(op1->gtVNPair.GetConservative()) &&
            curAssertion->op2.vn == vnStore->VNNormVal(op2->gtVNPair.GetConservative()))
        {
            return assertionIndex;
        }
    }
...
GenTree* Compiler::optAssertionProp_BndsChk(ASSERT_VALARG_TP assertions, GenTree* tree, GenTree* stmt)
{
    ...
    while (iter.NextElem(&index))
    {
        ...
        if (curAssertion->op1.bnd.vnLen ==
            vnStore->VNNormVal(vnStore->VNNormVal(arrBndsChk->gtArrLen->gtVNPair.GetConservative())))
        {
            ValueNum vnCurIdx = vnStore->VNNormVal(arrBndsChk->gtIndex->gtVNPair.GetConservative());
    ...
    if ((oldTree->gtFlags & GTF_SIDE_EFFECT) != 0)
    {
        ...
        if (oldTree == newTree)
        {
            ...
            assert(!gtNodeHasSideEffects(oldTree, GTF_EXCEPT) ||
                   vnStore->IsVNConstant(vnStore->VNNormVal(oldTree->gtVNPair.GetConservative())));
...
GenTree* Compiler::optVNConstantPropOnJTrue(BasicBlock* block, GenTree* stmt, GenTree* test)
{
    ...
    if (!vnStore->IsVNConstant(vnStore->VNNormVal(relop->gtVNPair.GetConservative())))
    {
        return nullptr;
    }
    ...
    ValueNum vnCns       = vnStore->VNNormVal(relop->gtVNPair.GetConservative());
