        ...
        for (Statement* stmt : block->Statements())
        {
            for (GenTree* tree = stmt->m_treeList; tree != nullptr; tree = tree->gtNext)
            {
                if (tree->gtOper == GT_JTRUE)
                {
                    ...
                    assert((tree->gtNext == nullptr) && (stmt->m_next == nullptr));
                    jtrue = tree;
                    break;
                }
...
Statement* Compiler::optVNAssertionPropCurStmt(BasicBlock* block, Statement* stmt)
{
    ...
    Statement* prev = (stmt == block->firstStmt()) ? nullptr : stmt->m_prev;
    ...
    Statement* nextStmt = (prev == nullptr) ? block->firstStmt() : prev->m_next;
    return nextStmt;
}
...
void Compiler::optAssertionPropMain()
{
    ...
    for (BasicBlock* block = fgFirstBB; block; block = block->bbNext)
    {
        ...
        while (stmt != nullptr)
        {
            ...
            Statement* prevStmt = (stmt == block->firstStmt()) ? nullptr : stmt->m_prev;
            ...
            for (GenTree* tree = stmt->m_treeList; tree != nullptr; tree = tree->gtNext)
            {
                ...
                JITDUMP("Propagating %s assertions for " FMT_BB ", stmt [%06d], tree [%06d], tree -> %d\n",
                        BitVecOps::ToString(apTraits, assertions), block->bbNum, dspStmtID(stmt), dspTreeID(tree),
            ...
            // Check if propagation removed statements starting from current stmt.
            // If so, advance to the next good statement.
            Statement* nextStmt = (prevStmt == nullptr) ? block->firstStmt() : prevStmt->m_next;
            stmt                = (stmt == nextStmt) ? stmt->m_next : nextStmt;
        }
