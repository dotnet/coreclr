...
void CodeGen::genPutArgStk(GenTreePtr treeNode)
{
    ...
    var_types  targetType = treeNode->TypeGet();
    GenTreePtr source     = treeNode->gtOp.gtOp1;  // was called 'data' previously 
    emitter *  emit       = getEmitter();
    ...
    else  // We have some kind of a struct argument
    {
        ...
        else  // We must have a GT_OBJ or a GT_LCL_VAR 
        {
            ...
            // We will copy this struct to the stack, possibly using a ldp instriction
            // Setup loReg and hiReg from the internal registers that wereserved in lower.
            //
            ...
            BYTE     gcPtrs[MAX_MULTIREG_ARGS] = {TYPE_GC_NONE, TYPE_GC_NONE, TYPE_GC_NONE, TYPE_GC_NONE };
            ...
            else  // addrNode is used
            {
                ...
                hfaType    = compiler->GetHfaType(objClass);   // set to float or double if it is an HFA, otherwise TYP_UNDEF
            ...
            //             str     x3, [sp, #40]
            //
            // Note that when loadiing from a varNode we currently can't use the ldp instruction
            ...
            while (remainingSize > 0)
            {
                ...
                else // (remainingSize < TARGET_POINTER_SIZE)
                {
                    ...
                    // Emit a store instructions to store the register into the outgoing argument area
