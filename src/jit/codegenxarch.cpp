...
void CodeGen::genCallInstruction(GenTreeCall* call)
{
    ...
    {
        inst_RV_IV(INS_sub, REG_SPBASE, stackArgBytes, EA_PTRSIZE);
    }
#else
                    ...
                    switch (fieldNode->OperGet())
                    {
                        case GT_LCL_VAR:
                            ...
                            // mov      gword ptr [esp+argOffset], eax
                            {
                                ...
                                getEmitter()->emitIns_R_S(ins_Store(fieldType), typeSize, REG_SCRATCH, fieldLclNum, 0);
                                getEmitter()->emitIns_AR_R(ins_Store(fieldType), EA_4BYTE, REG_SCRATCH, REG_SPBASE,
    ...
    else
    {
        ...
        for (int i = numSlots - 1; i >= 0; --i)
        {
            ...
            if (srcAddrInReg)
            {
                ...
                getEmitter()->emitIns_AR_R(ins_Store(TYP_INT), EA_4BYTE, REG_SCRATCH, REG_SPBASE, argOffset + offset);
            ...
            else
            {
                ...
                getEmitter()->emitIns_R_S(ins_Store(TYP_INT), slotAttr, REG_SCRATCH, srcLclNum, srcLclOffset + offset);
                getEmitter()->emitIns_AR_R(ins_Store(TYP_INT), EA_4BYTE, REG_SCRATCH, REG_SPBASE, argOffset + offset);
            }
#else  // FEATURE_FIXED_OUT_ARGS
            if (srcAddrInReg)
