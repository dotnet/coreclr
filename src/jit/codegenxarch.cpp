    ...
    case GT_RETURNTRAP:
        {
            ...
            genEmitHelperCall(CORINFO_HELP_STOP_FOR_GC, 0, insCallReturnRegisterTypes(EA_UNKNOWN), tmpReg);
            genDefineTempLabel(skipLabel);
        }
        break;
    ...
    if (target != nullptr)
    {
        if (target->isContainedIndir())
        {
            ...
            else
            {
                ...
                GenEmitCallInfo genEmitCallInfo = GenEmitCallInfo(GenCallTypeToEmit::GEN_EMIT_CALL_INDIRECT,
...
void        CodeGen::genEmitHelperCall(unsigned    helper,
                                       int         argSize,
                                       insCallReturnRegisterTypes callReturnType
