...
protected:
    ...
    public:
        ...
        LoopHoistContext(Compiler* comp)
            : m_pHoistedInCurLoop(nullptr), m_hoistedInParentLoops(comp->getAllocatorLoopHoist()),
...
#define VSQUIRK_STACK_PAD (2 * REGSIZE_BYTES)
    ...
    struct MemStats
    {
        ...
        MemStats(const MemStats& ms)
            : allocCnt(ms.allocCnt), allocSz(ms.allocSz), allocSzMax(ms.allocSzMax),
              nraTotalSizeAlloc(ms.nraTotalSizeAlloc), nraTotalSizeUsed(ms.nraTotalSizeUsed)
        {
            for (int i = 0; i < CMK_Count; i++)
            {
