...
void Compiler::fgInsertStmtNearEnd(BasicBlock* block, Statement* stmt)
{
    ...
    if ((block->bbJumpKind == BBJ_COND) || (block->bbJumpKind == BBJ_SWITCH) || (block->bbJumpKind == BBJ_RETURN))
    {
        ...
        {
            assert(lastStmt->GetRootTree()->gtOper == GT_JTRUE);
        }
        else if (block->bbJumpKind == BBJ_RETURN)
        {
            assert((lastStmt->GetRootTree()->gtOper == GT_RETURN) || (lastStmt->GetRootTree()->gtOper == GT_JMP) ||
                   ...
                   ((lastStmt->GetRootTree()->gtOper == GT_CALL) &&
                    ((info.compRetType == TYP_VOID) || lastStmt->GetRootTree()->AsCall()->IsTailCall())));
        }
        else
        {
            assert(block->bbJumpKind == BBJ_SWITCH);
            assert(lastStmt->GetRootTree()->gtOper == GT_SWITCH);
        }
#endif // DEBUG
...
GenTree* Compiler::fgCreateMonitorTree(unsigned lvaMonAcquired, unsigned lvaThisVar, BasicBlock* block, bool enter)
{
    ...
    if (block->bbJumpKind == BBJ_RETURN && block->lastStmt()->GetRootTree()->gtOper == GT_RETURN)
    {
        GenTree* retNode = block->lastStmt()->GetRootTree();
...
private:
        ...
        if ((returnBlock != nullptr) && (maxReturns > 1) && !comp->opts.compDbgCode)
        {
            ...
            if (retConst != nullptr)
            {
                ...
                if (constReturnBlock != nullptr)
                {
                    ...
                    assert(returnBlock->lastStmt()->GetRootTree()->OperIs(GT_RETURN));
                    assert(returnBlock->lastStmt()->GetRootTree()->gtGetOp1()->IsIntegralConst());
    ...
    static GenTreeIntConCommon* GetReturnConst(BasicBlock* returnBlock)
    {
        ...
        GenTree* lastExpr = lastStmt->GetRootTree();
...
#ifdef DEBUG
    if (verbose &&
        stmt->GetRootTree()->gtOper != GT_NOP) // Don't print if it is a GT_NOP. Too much noise from the inliner.
    {
        printf("\nRemoving statement ");
        gtDispStmt(stmt);
...
bool Compiler::fgOptimizeSwitchBranches(BasicBlock* block)
{
    ...
    else
    {
        ...
        switchTree = switchStmt->GetRootTree();
    ...
    if (block->NumSucc(this) == 1)
    {
        ...
        else
        {
            ...
            if (switchTree->gtFlags & GTF_SIDE_EFFECT)
            {
                ...
                switchStmt->SetRootTree(sideEffList);
...
bool Compiler::fgBlockEndFavorsTailDuplication(BasicBlock* block)
{
    ...
    else
    {
        ...
        Statement* lastStmt = block->lastStmt();
        GenTree*   tree     = lastStmt->GetRootTree();
...
bool Compiler::fgOptimizeUncondBranchToSimpleCond(BasicBlock* block, BasicBlock* target)
{
    ...
    GenTree* cloned = gtCloneExpr(stmt->GetRootTree());
    noway_assert(cloned);
...
bool Compiler::fgOptimizeBranchToNext(BasicBlock* block, BasicBlock* bNext, BasicBlock* bPrev)
{
    ...
    else
    {
        ...
        else
        {
            ...
            GenTree*   cond     = condStmt->GetRootTree();
            ...
            if (cond->gtFlags & GTF_SIDE_EFFECT)
            {
                ...
                else
                {
                    ...
                    condStmt->SetRootTree(sideEffList);
...
bool Compiler::fgOptimizeBranch(BasicBlock* bJump)
{
    ...
    GenTree* condTree = newLastStmt->GetRootTree();
...
void Compiler::fgReorderBlocks()
{
    ...
    for (bPrev = fgFirstBB, block = bPrev->bbNext; block != nullptr; bPrev = block, block = block->bbNext)
    {
        ...
        if (bPrev->bbJumpKind == BBJ_COND)
        {
            ...
            GenTree* condTest = condTestStmt->GetRootTree();
...
void Compiler::fgSetStmtSeq(Statement* stmt)
{
    ...
    fgSetTreeSeqHelper(stmt->GetRootTree(), false);
...
void Compiler::fgDebugCheckNodeLinks(BasicBlock* block, Statement* stmt)
{
    ...
    for (GenTree* tree = stmt->GetTreeList(); tree != nullptr; tree = tree->gtNext)
    {
        ...
        {
            noway_assert(tree == stmt->GetRootTree());
        }
        ...
        if (tree->OperIsLeaf())
        {
            if (tree->gtOper == GT_CATCH_ARG)
            {
                ...
                noway_assert(stmt->GetRootTree()->gtFlags & GTF_ORDER_SIDEEFF);
            }
        }
...
void Compiler::fgDebugCheckStmtsList(BasicBlock* block, bool morphTrees)
{
    for (Statement* stmt : block->Statements())
    {
        ...
        fgDebugCheckFlags(stmt->GetRootTree());
...
void Compiler::fgInline()
{
    ...
    do
    {
        ...
        for (Statement* stmt : block->Statements())
        {
            ...
            fgWalkTreePre(stmt->GetRootTreePointer(), fgFindNonInlineCandidate, stmt);
#endif
            ...
            if (expr->IsCall())
            {
                ...
                if (call->IsInlineCandidate() || call->IsGuardedDevirtualizationCandidate())
                {
                    ...
                    if (stmt->GetRootTree()->IsNothingNode())
                    {
                        fgRemoveStmt(block, stmt);
                        continue;
                    }
            ...
            fgWalkTree(stmt->GetRootTreePointer(), fgUpdateInlineReturnExpressionPlaceHolder, fgLateDevirtualization,
                       (void*)this);
            ...
            {
                stmt->SetRootTree(expr->gtOp.gtOp1);
            }
        }
    ...
    do
    {
        for (Statement* stmt : block->Statements())
        {
            ...
            fgWalkTreePre(stmt->GetRootTreePointer(), fgDebugCheckInlineCandidates);
        }
...
void Compiler::fgInsertInlineeBlocks(InlineInfo* pInlineInfo)
{
    ...
    noway_assert(iciStmt->GetRootTree() != nullptr);
    assert(iciStmt->GetRootTree() == iciCall);
...
// Update optMethodFlags
    ...
    //
    iciStmt->SetRootTree(gtNewNothingNode());
}
        ...
        for (BasicBlock* block = firstHandlerBlock; block != endHandlerBlock; block = block->bbNext)
        {
            ...
            if (block->getHndIndex() == XTnum)
            {
                ...
                if (block->bbJumpKind == BBJ_EHFINALLYRET)
                {
                    ...
                    GenTree*   finallyRetExpr = finallyRet->GetRootTree();
...
unsigned Compiler::fgMeasureIR()
{
    ...
    for (BasicBlock* block = fgFirstBB; block != nullptr; block = block->bbNext)
    {
        if (!block->IsLIR())
        {
            for (Statement* stmt : block->Statements())
            {
                fgWalkTreePre(stmt->GetRootTreePointer(),
