...
Statement* Compiler::fgInsertStmtNearEnd(BasicBlock* block, Statement* stmt)
{
    ...
    if ((block->bbJumpKind == BBJ_COND) || (block->bbJumpKind == BBJ_SWITCH) || (block->bbJumpKind == BBJ_RETURN))
    {
        ...
        noway_assert(lastStmt != nullptr && lastStmt->m_next == nullptr);
        Statement* insertionPoint = lastStmt->m_prev;

#if DEBUG
        if (block->bbJumpKind == BBJ_COND)
        {
            assert(lastStmt->m_rootTree->gtOper == GT_JTRUE);
        }
        else if (block->bbJumpKind == BBJ_RETURN)
        {
            assert((lastStmt->m_rootTree->gtOper == GT_RETURN) || (lastStmt->m_rootTree->gtOper == GT_JMP) ||
                   ...
                   ((lastStmt->m_rootTree->gtOper == GT_CALL) &&
                    ((info.compRetType == TYP_VOID) || lastStmt->m_rootTree->AsCall()->IsTailCall())));
        }
        else
        {
            assert(block->bbJumpKind == BBJ_SWITCH);
            assert(lastStmt->m_rootTree->gtOper == GT_SWITCH);
        }
#endif // DEBUG

        // Append 'stmt' before 'lastStmt'.
        stmt->m_next     = lastStmt;
        lastStmt->m_prev = stmt;
        ...
        else
        {
            ...
            noway_assert(insertionPoint != nullptr && (insertionPoint->m_next == lastStmt));
...
Statement* Compiler::fgInsertStmtAfter(BasicBlock* block, Statement* insertionPoint, Statement* stmt)
{
    ...
    if (insertionPoint->m_next == nullptr)
    {
        ...
        assert(block->bbStmtList->m_prev == insertionPoint);
        block->bbStmtList->m_prev = stmt;
    }
    else
    {
        stmt->m_next = insertionPoint->m_next;
        stmt->m_prev = insertionPoint;
...
Statement* Compiler::fgInsertStmtBefore(BasicBlock* block, Statement* insertionPoint, Statement* stmt)
{
    ...
    {
        stmt->m_next = insertionPoint;
        stmt->m_prev = insertionPoint->m_prev;
                                           ...
                                           Statement* stmtList)
{
    ...
    if (stmtNext == nullptr)
    {
        stmtAfter->m_next         = stmtList;
        stmtList->m_prev          = stmtAfter;
        block->bbStmtList->m_prev = stmtLast;
        goto _Done;
    }
    ...
    stmtLast->m_next = stmtNext;
    stmtNext->m_prev = stmtLast;

_Done:

    noway_assert(block->bbStmtList == nullptr || block->bbStmtList->m_prev->m_next == nullptr);
        ...
        switch (opcode)
        {
            ...
            case CEE_CALLI:
            {
                if (compIsForInlining() ||               // Ignore tail call in the inlinee. Period.
                    ...
                    //     noway_assert(fgMorphStmt->m_next == NULL);
                    )
                {
...
GenTree* Compiler::fgCreateMonitorTree(unsigned lvaMonAcquired, unsigned lvaThisVar, BasicBlock* block, bool enter)
{
    ...
    if (block->bbJumpKind == BBJ_RETURN && block->lastStmt()->m_rootTree->gtOper == GT_RETURN)
    {
        GenTree* retNode = block->lastStmt()->m_rootTree;
...
private:
        ...
        if ((returnBlock != nullptr) && (maxReturns > 1) && !comp->opts.compDbgCode)
        {
            ...
            if (retConst != nullptr)
            {
                ...
                if (constReturnBlock != nullptr)
                {
                    ...
                    assert(returnBlock->lastStmt()->m_rootTree->OperIs(GT_RETURN));
                    assert(returnBlock->lastStmt()->m_rootTree->gtGetOp1()->IsIntegralConst());
    ...
    static GenTreeIntConCommon* GetReturnConst(BasicBlock* returnBlock)
    {
        ...
        GenTree* lastExpr = lastStmt->m_rootTree;
...
void Compiler::fgRemoveStmt(BasicBlock* block, Statement* stmt)
{
...
#ifdef DEBUG
    if (verbose &&
        stmt->m_rootTree->gtOper != GT_NOP) // Don't print if it is a GT_NOP. Too much noise from the inliner.
    {
        printf("\nRemoving statement ");
    ...
    {
        stmt->m_prev->m_next      = nullptr;
        block->bbStmtList->m_prev = stmt->m_prev;
    }
    ...
    else
    {
        ...
        if (stmtList1 != nullptr)
        {
            ...
            if (stmtList2 != nullptr)
            {
                ...
                stmtLast1->m_next = stmtList2;
                stmtList2->m_prev = stmtLast1;
                stmtList1->m_prev = stmtLast2;
            }
        }
        else
        {
...
/* Removing an unreachable block */
    ...
    else
    {
        ...
        if (block->bbStmtList != firstNonPhi)
        {
            ...
            {
                firstNonPhi->m_prev = block->lastStmt();
            }
...
bool Compiler::fgOptimizeSwitchBranches(BasicBlock* block)
{
    ...
    else
    {
        ...
        switchTree = switchStmt->m_rootTree;
    ...
    if (block->NumSucc(this) == 1)
    {
        ...
        else
        {
            ...
            if (switchTree->gtFlags & GTF_SIDE_EFFECT)
            {
                ...
                switchStmt->m_rootTree = sideEffList;
...
bool Compiler::fgBlockEndFavorsTailDuplication(BasicBlock* block)
{
    ...
    else
    {
        ...
        Statement* lastStmt = block->lastStmt();
        GenTree*   tree     = lastStmt->m_rootTree;
...
bool Compiler::fgOptimizeBranchToNext(BasicBlock* block, BasicBlock* bNext, BasicBlock* bPrev)
{
    ...
    else
    {
        ...
        else
        {
            ...
            GenTree*   cond     = condStmt->m_rootTree;
            ...
            if (cond->gtFlags & GTF_SIDE_EFFECT)
            {
                ...
                else
                {
                    ...
                    condStmt->m_rootTree = sideEffList;
...
bool Compiler::fgOptimizeBranch(BasicBlock* bJump)
{
    ...
    for (Statement* curStmt = bDest->firstStmt(); curStmt != nullptr; curStmt = curStmt->m_next)
    {
        ...
        stmt->m_prev = newLastStmt;
        newLastStmt  = stmt;
    }

    // Get to the condition node from the statement tree.
    GenTree* condTree = newLastStmt->m_rootTree;
    ...
    if (lastStmt != nullptr)
    {
        ...
        stmt->m_prev        = newLastStmt;
        lastStmt->m_next    = newStmtList;
        newStmtList->m_prev = lastStmt;
    }
    else
    {
        bJump->bbStmtList   = newStmtList;
        newStmtList->m_prev = newLastStmt;
    }
...
void Compiler::fgReorderBlocks()
{
    ...
    for (bPrev = fgFirstBB, block = bPrev->bbNext; block != nullptr; bPrev = block, block = block->bbNext)
    {
        ...
        if (bPrev->bbJumpKind == BBJ_COND)
        {
            ...
            GenTree* condTest = condTestStmt->m_rootTree;
...
void Compiler::fgSetStmtSeq(Statement* stmt)
{
    ...
    fgSetTreeSeqHelper(stmt->m_rootTree, false);

    /* Record the address of the first node */

    stmt->m_treeList = fgTreeSeqBeg;
...
void Compiler::fgSetBlockOrder(BasicBlock* block)
{
    for (Statement* stmt = block->firstStmt(); stmt != nullptr; stmt = stmt->m_next)
    {
        ...
        if (block->bbStmtList == stmt)
        {
            ...
            assert(stmt->m_prev->m_next == nullptr);
        }
        else
        {
            assert(stmt->m_prev->m_next == stmt);
        }

        assert(stmt->m_next->m_prev == stmt);
#endif // DEBUG
    }
}
...
void Compiler::fgDebugCheckNodeLinks(BasicBlock* block, Statement* stmt)
{
    ...
    // terminates.
    assert(stmt->m_treeList->gtPrev == nullptr);
    ...
    for (GenTree* tree = stmt->m_treeList; tree != nullptr; tree = tree->gtNext)
    {
        ...
        {
            noway_assert(tree == stmt->m_treeList);
        }
        ...
        {
            noway_assert(tree == stmt->m_rootTree);
        }
        ...
        if (tree->OperIsLeaf())
        {
            if (tree->gtOper == GT_CATCH_ARG)
            {
                ...
                noway_assert(stmt->m_treeList->gtOper == GT_CATCH_ARG);
                // The root of the tree should have GTF_ORDER_SIDEEFF set
                noway_assert(stmt->m_rootTree->gtFlags & GTF_ORDER_SIDEEFF);
            }
        }
...
void Compiler::fgDebugCheckStmtsList(BasicBlock* block, bool morphTrees)
{
    for (Statement* stmt = block->firstStmt(); stmt != nullptr; stmt = stmt->m_next)
    {
        ...
        * last statement in a block is nullptr. */
        ...
        noway_assert(stmt->m_prev);

        if (stmt == block->bbStmtList)
        {
            noway_assert(stmt->m_prev->m_next == nullptr);
        }
        else
        {
            noway_assert(stmt->m_prev->m_next == stmt);
        }

        if (stmt->m_next)
        {
            noway_assert(stmt->m_next->m_prev == stmt);
        }
        else
        {
            noway_assert(block->lastStmt() == stmt);
        ...
        fgDebugCheckFlags(stmt->m_rootTree);
...
void Compiler::fgInline()
{
    ...
    do
    {
        ...
        for (Statement* stmt = block->firstStmt(); stmt != nullptr; stmt = stmt->m_next)
        {
            ...
            fgWalkTreePre(&stmt->m_rootTree, fgFindNonInlineCandidate, stmt);
#endif
            ...
            if (expr->IsCall())
            {
                ...
                if (call->IsInlineCandidate() || call->IsGuardedDevirtualizationCandidate())
                {
                    ...
                    if (stmt->m_rootTree->IsNothingNode())
                    {
                        fgRemoveStmt(block, stmt);
                        continue;
                    }
            ...
            fgWalkTree(&stmt->m_rootTree, fgUpdateInlineReturnExpressionPlaceHolder, fgLateDevirtualization,
                       (void*)this);
            ...
            {
                stmt->m_rootTree = expr->gtOp.gtOp1;
            }
        }
...
void Compiler::fgInsertInlineeBlocks(InlineInfo* pInlineInfo)
{
    ...
    noway_assert(iciStmt->m_rootTree != nullptr);
    assert(iciStmt->m_rootTree == iciCall);
            ...
            if (verbose)
            {
                ...
                if (currentDumpStmt != stmtAfter)
                {
                    do
                    {
                        ...
                        gtDispTree(currentDumpStmt->m_rootTree);
                        printf("\n");
    ...
    else
    {
        ...
        topBlock_End->m_next = nullptr;

        // Fix up all the pointers.
        topBlock->bbStmtList         = topBlock_Begin;
        topBlock->bbStmtList->m_prev = topBlock_End;

        bottomBlock->bbStmtList         = bottomBlock_Begin;
        bottomBlock->bbStmtList->m_prev = bottomBlock_End;
    }
...
// Update optMethodFlags
    ...
    //
    iciStmt->m_rootTree = gtNewNothingNode();
}
        ...
        for (unsigned argNum = 0; argNum < inlineInfo->argCnt; argNum++)
        {
            ...
            if (argInfo.argHasTmp)
            {
                ...
                else
                {
                    ...
                    if (verbose)
                    {
                        gtDispTree(afterStmt->m_rootTree);
                    }
#endif // DEBUG
                }
            }
...
void Compiler::fgInlineAppendStatements(InlineInfo* inlineInfo, BasicBlock* block, Statement* stmtAfter)
{
    ...
    for (unsigned lclNum = 0; lclNum < lclCnt; lclNum++)
    {
        ...
        if (verbose)
        {
            gtDispTree(nullStmt->m_rootTree);
        }
#endif // DEBUG
    }
        ...
        for (BasicBlock* block = firstHandlerBlock; block != endHandlerBlock; block = block->bbNext)
        {
            ...
            if (block->getHndIndex() == XTnum)
            {
                ...
                if (block->bbJumpKind == BBJ_EHFINALLYRET)
                {
                    ...
                    GenTree*   finallyRetExpr = finallyRet->m_rootTree;
