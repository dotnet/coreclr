    ...
    switch (oper)
    {
        ...
        case GT_CALL:
            ...
            if (tree->gtCall.gtCallType == CT_INDIRECT)
            {
                ...
                assert(tree->gtCall.gtCallCookie == NULL || tree->gtCall.gtCallCookie->gtOper == GT_CNS_INT ||
...
GenTree::VtablePtr GenTree::GetVtableForOper(genTreeOps oper)
{
    ...
    if (s_vtablesForOpers[oper] != NULL)
...
GenTreeQmark::GenTreeQmark(var_types type, GenTreePtr cond, GenTreePtr colonOp, Compiler* comp)
    : GenTreeOp(GT_QMARK, type, cond, colonOp), gtThenLiveSet(VarSetOps::UninitVal()),
    ...
    switch (oper)
    {
        ...
        case GT_CALL:
            ...
            copy->gtCall.gtCallObjp =
                tree->gtCall.gtCallObjp ? gtCloneExpr(tree->gtCall.gtCallObjp, addFlags, varNum, varVal) : NULL;
            ...
            if (tree->gtCall.gtCallType == CT_INDIRECT)
            {
                copy->gtCall.gtCallCookie =
                    tree->gtCall.gtCallCookie ? gtCloneExpr(tree->gtCall.gtCallCookie, addFlags, varNum, varVal) : NULL;
                copy->gtCall.gtCallAddr =
                    tree->gtCall.gtCallAddr ? gtCloneExpr(tree->gtCall.gtCallAddr, addFlags, varNum, varVal) : NULL;
            }
        ...
        case GT_FIELD:
            ...
            copy = gtNewFieldRef(tree->TypeGet(), tree->gtField.gtFldHnd, NULL, tree->gtField.gtFldOffset);

            copy->gtField.gtFldObj =
                tree->gtField.gtFldObj ? gtCloneExpr(tree->gtField.gtFldObj, addFlags, varNum, varVal) : 0;
            copy->gtField.gtFldMayOverlap = tree->gtField.gtFldMayOverlap;
#ifdef FEATURE_READYTORUN_COMPILER
...
GenTreeOperandIterator::GenTreeOperandIterator()
    : m_node(nullptr), m_operand(nullptr), m_argList(nullptr), m_multiRegArg(nullptr), m_expandMultiRegArgs(false),
      m_state(-1)
{
}

GenTreeOperandIterator::GenTreeOperandIterator(GenTree* node, bool expandMultiRegArgs)
    : m_node(node), m_operand(nullptr), m_argList(nullptr), m_multiRegArg(nullptr),
      m_expandMultiRegArgs(expandMultiRegArgs), m_state(0)
{
                          ...
                          bool                             topOnly) /* = false */
{
    ...
    switch (tree->gtOper)
    {
        ...
        case GT_CALL:
        {
            ...
            if (((tree->gtFlags & GTF_CALL_INLINE_CANDIDATE) != 0) && (tree->gtCall.gtInlineCandidateInfo != NULL) &&
            ...
            if (!topOnly)
            {
                ...
                if ((tree->gtCall.gtCallObjp != NULL) && (tree->gtCall.gtCallObjp->gtOper != GT_NOP) &&
...
{
    bool res = OperGet() == GT_ASG && gtOp.gtOp2 != NULL && gtOp.gtOp2->OperGet() == GT_PHI;
...
// static
FieldSeqNode FieldSeqStore::s_notAField(NULL,
                                        NULL); // Value doesn't matter; exists only to provide a distinguished address.
