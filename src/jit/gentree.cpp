...
void GenTree::DumpFieldLayout(FILE* fp)
{
    ...
    // Dump data member sizes + offsets for important GenTree flavors

    size_t __lastOffs;

#define INIT_FLD_OFNB(t) \
    fprintf(fp, "\nFields of " #t ":\n"); \
    __lastOffs = 0;

#define INIT_FLD_OFFS(t,b) \
    fprintf(fp, "\nFields of " #t " (base=" #b "):\n"); \
    __lastOffs = sizeof(b);

#define __DUMP_FIELD(t,m,o,w) \
    if (__lastOffs != o) fprintf(fp, "                       ..............%3u byte(s) of padding\n", o - __lastOffs); \
    __lastOffs = o+w; \
    fprintf(fp, "  %-20s is at %3u (size=%u)\n", m, o, w);

#define SHOW_FLD_OFFS(t,m) __DUMP_FIELD(t, #m, offsetof(t,m), sizeof(((t*)0)->m))
#define SHOW_FLD_BITS(t,b) __DUMP_FIELD(t, "...bitfield(s)...", ((__lastOffs+sizeof(b)-1) & ~(sizeof(b)-1)), sizeof(b))

#define DONE_FLD_OFFS(t) \
    if (__lastOffs != sizeof(t)) fprintf(fp, "                       .............. %2u byte(s) of padding\n", sizeof(t) - __lastOffs); \
    fprintf(fp, "  ---------------------------------------\n"); \
    fprintf(fp, "  Size of %-15s is %3u\n", #t, sizeof(t));

    ///////////////////////////////////////////////

    INIT_FLD_OFNB(GenTree)

    SHOW_FLD_OFFS(GenTree, gtOper)
    SHOW_FLD_OFFS(GenTree, gtType)
    SHOW_FLD_OFFS(GenTree, gtCSEnum)
    SHOW_FLD_OFFS(GenTree, gtLIRFlags)
#if ASSERTION_PROP
    SHOW_FLD_OFFS(GenTree, gtAssertionNum)
#endif
#if FEATURE_STACK_FP_X87
    SHOW_FLD_OFFS(GenTree, gtFPlvl)
#endif
    SHOW_FLD_OFFS(GenTree, _gtCostEx)
    SHOW_FLD_OFFS(GenTree, _gtCostSz)
    SHOW_FLD_OFFS(GenTree, _gtRegNum)           // union with _gtRegPair
    SHOW_FLD_OFFS(GenTree, gtFlags)
    SHOW_FLD_OFFS(GenTree, gtVNPair)
    SHOW_FLD_OFFS(GenTree, gtRsvdRegs)
#ifdef LEGACY_BACKEND
    SHOW_FLD_OFFS(GenTree, gtUsedRegs)
#else
    SHOW_FLD_OFFS(GenTree, gtLsraInfo)
#endif
#if MOVE_ILOFFS_INTO_BASE
    SHOW_FLD_OFFS(GenTree, gtLclOffs)           // union with gtLclILoffs/gtStmtILoffsx/gtBlkOpGcUnsafe
#endif
    SHOW_FLD_OFFS(GenTree, gtNext)
    SHOW_FLD_OFFS(GenTree, gtPrev)

    DONE_FLD_OFFS(GenTree)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeUnOp, GenTree)
    SHOW_FLD_OFFS(GenTreeUnOp, gtOp1)
    DONE_FLD_OFFS(GenTreeUnOp)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeOp, GenTreeUnOp)
    SHOW_FLD_OFFS(GenTreeOp, gtOp2)
    DONE_FLD_OFFS(GenTreeOp)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeIntConCommon, GenTree)
    // no fields
    DONE_FLD_OFFS(GenTreeIntConCommon)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeIntCon, GenTreeIntConCommon)
    SHOW_FLD_OFFS(GenTreeIntCon, gtIconVal)
    SHOW_FLD_OFFS(GenTreeIntCon, gtCompileTimeHandle)
    SHOW_FLD_OFFS(GenTreeIntCon, gtFieldSeq)
#if defined (LATE_DISASM)
    SHOW_FLD_OFFS(GenTreeIntCon, gtIconHdl)     // union with gtIconFld
#endif
    DONE_FLD_OFFS(GenTreeIntCon)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeLngCon, GenTreeIntConCommon)
    SHOW_FLD_OFFS(GenTreeLngCon, gtLconVal)
    DONE_FLD_OFFS(GenTreeLngCon)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeCast, GenTreeOp)
#if!MOVE_ILOFFS_INTO_BASE
    SHOW_FLD_OFFS(GenTreeCast, gtCastType)
#endif
    DONE_FLD_OFFS(GenTreeCast)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeQmark, GenTreeOp)
    SHOW_FLD_OFFS(GenTreeQmark, gtThenLiveSet)
    SHOW_FLD_OFFS(GenTreeQmark, gtElseLiveSet)
    DONE_FLD_OFFS(GenTreeQmark)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeIntrinsic, GenTreeOp)
#if!MOVE_ILOFFS_INTO_BASE
    SHOW_FLD_OFFS(GenTreeIntrinsic, gtIntrinsicId)
#endif
    SHOW_FLD_OFFS(GenTreeIntrinsic, gtMethodHandle)
#ifdef FEATURE_READYTORUN_COMPILER
    SHOW_FLD_OFFS(GenTreeIntrinsic, gtEntryPoint)
#endif
    DONE_FLD_OFFS(GenTreeIntrinsic)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeBoundsChk, GenTree)
    SHOW_FLD_OFFS(GenTreeBoundsChk, gtArrLen)
    SHOW_FLD_OFFS(GenTreeBoundsChk, gtIndex)
    SHOW_FLD_OFFS(GenTreeBoundsChk, gtIndRngFailBB)
    SHOW_FLD_OFFS(GenTreeBoundsChk, gtThrowKind)
    SHOW_FLD_OFFS(GenTreeBoundsChk, gtStkDepth)
    DONE_FLD_OFFS(GenTreeBoundsChk)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeIndex, GenTreeOp)
#if!MOVE_ILOFFS_INTO_BASE
    SHOW_FLD_OFFS(GenTreeIndex, gtIndElemSize)
#endif
    SHOW_FLD_OFFS(GenTreeIndex, gtStructElemClass)
    DONE_FLD_OFFS(GenTreeIndex)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeLclVarCommon, GenTreeUnOp)
    SHOW_FLD_OFFS(GenTreeLclVarCommon, _gtLclNum)
    SHOW_FLD_OFFS(GenTreeLclVarCommon, _gtSsaNum)
    DONE_FLD_OFFS(GenTreeLclVarCommon)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeLclVar, GenTreeLclVarCommon)
#if!MOVE_ILOFFS_INTO_BASE
    SHOW_FLD_OFFS(GenTreeLclVar, gtLclILoffs)
#endif
    DONE_FLD_OFFS(GenTreeLclVar);

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeLclFld, GenTreeLclVarCommon)
#if!MOVE_ILOFFS_INTO_BASE
    SHOW_FLD_OFFS(GenTreeLclFld, gtLclOffs)
#endif
    SHOW_FLD_OFFS(GenTreeLclFld, gtFieldSeq)
    DONE_FLD_OFFS(GenTreeLclFld);

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeRegVar, GenTreeLclVarCommon)
    SHOW_FLD_OFFS(GenTreeRegVar, _gtRegNum)
    DONE_FLD_OFFS(GenTreeRegVar);

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeField, GenTree)
    SHOW_FLD_OFFS(GenTreeField, gtFldObj)
    SHOW_FLD_OFFS(GenTreeField, gtFldHnd)
    SHOW_FLD_OFFS(GenTreeField, gtFldOffset)
    SHOW_FLD_OFFS(GenTreeField, gtFldMayOverlap)
#ifdef FEATURE_READYTORUN_COMPILER
    SHOW_FLD_OFFS(GenTreeField, gtFieldLookup)
#endif
    DONE_FLD_OFFS(GenTreeField)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeArrOffs, GenTree)
    SHOW_FLD_OFFS(GenTreeArrOffs, gtOffset)
    SHOW_FLD_OFFS(GenTreeArrOffs, gtIndex)
    SHOW_FLD_OFFS(GenTreeArrOffs, gtArrObj)
    SHOW_FLD_OFFS(GenTreeArrOffs, gtCurrDim)
    SHOW_FLD_OFFS(GenTreeArrOffs, gtArrRank)
    SHOW_FLD_OFFS(GenTreeArrOffs, gtArrElemType)
    DONE_FLD_OFFS(GenTreeArrOffs)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeArrElem, GenTree)
    SHOW_FLD_OFFS(GenTreeArrElem, gtArrObj)
    SHOW_FLD_OFFS(GenTreeArrElem, gtArrInds)
    SHOW_FLD_OFFS(GenTreeArrElem, gtArrRank)
    SHOW_FLD_OFFS(GenTreeArrElem, gtArrElemSize)
    SHOW_FLD_OFFS(GenTreeArrElem, gtArrElemType)
    DONE_FLD_OFFS(GenTreeArrElem)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeStmt, GenTree)
    SHOW_FLD_OFFS(GenTreeStmt, gtStmtExpr)
    SHOW_FLD_OFFS(GenTreeStmt, gtStmtList)
    SHOW_FLD_OFFS(GenTreeStmt, gtInlineContext)
#if defined(DEBUGGING_SUPPORT) || defined(DEBUG)
#if!MOVE_ILOFFS_INTO_BASE
    SHOW_FLD_OFFS(GenTreeStmt, gtStmtILoffsx)
#endif
#endif
    DONE_FLD_OFFS(GenTreeStmt)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeIndir, GenTreeOp)
    DONE_FLD_OFFS(GenTreeIndir)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeBlk, GenTreeIndir)
    SHOW_FLD_OFFS(GenTreeBlk, gtBlkSize)
    SHOW_FLD_OFFS(GenTreeBlk, gtBlkOpKind)
#if!MOVE_ILOFFS_INTO_BASE
    SHOW_FLD_OFFS(GenTreeBlk, gtBlkOpGcUnsafe)
#endif
    DONE_FLD_OFFS(GenTreeBlk)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeAllocObj, GenTreeUnOp)
    SHOW_FLD_OFFS(GenTreeAllocObj, gtNewHelper)
    SHOW_FLD_OFFS(GenTreeAllocObj, gtAllocObjClsHnd)
    DONE_FLD_OFFS(GenTreeAllocObj)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeObj, GenTreeBlk)
    SHOW_FLD_OFFS(GenTreeObj, gtClass)
    SHOW_FLD_OFFS(GenTreeObj, gtGcPtrs)
    SHOW_FLD_OFFS(GenTreeObj, _gtGcPtrCount)
    SHOW_FLD_OFFS(GenTreeObj, gtSlots)
    DONE_FLD_OFFS(GenTreeObj)

    ///////////////////////////////////////////////

    INIT_FLD_OFFS(GenTreeCall, GenTree)

    SHOW_FLD_OFFS(GenTreeCall, gtCallObjp)
    SHOW_FLD_OFFS(GenTreeCall, gtCallArgs)
    SHOW_FLD_OFFS(GenTreeCall, gtCallLateArgs)
    SHOW_FLD_OFFS(GenTreeCall, fgArgInfo)
#if !FEATURE_FIXED_OUT_ARGS
    SHOW_FLD_OFFS(GenTreeCall, regArgListCount)
    SHOW_FLD_OFFS(GenTreeCall, regArgList)
#endif
    SHOW_FLD_OFFS(GenTreeCall, callSig)
#ifdef LEGACY_BACKEND
    SHOW_FLD_OFFS(GenTreeCall, gtCallRegUsedMask)
#endif
#if FEATURE_MULTIREG_RET
    SHOW_FLD_OFFS(GenTreeCall, gtReturnTypeDesc)
    SHOW_FLD_OFFS(GenTreeCall, gtOtherRegs)
    SHOW_FLD_OFFS(GenTreeCall, gtSpillFlags)
#endif
    SHOW_FLD_OFFS(GenTreeCall, gtCallMoreFlags)
    SHOW_FLD_BITS(GenTreeCall, unsigned char)
    SHOW_FLD_OFFS(GenTreeCall, gtRetClsHnd)
    SHOW_FLD_OFFS(GenTreeCall, gtCallCookie)    // union with gtInlineCandidateInfo, etc.
    SHOW_FLD_OFFS(GenTreeCall, gtControlExpr)
    SHOW_FLD_OFFS(GenTreeCall, gtCallAddr)      // union with gtCallMethHnd
#ifdef FEATURE_READYTORUN_COMPILER
    SHOW_FLD_OFFS(GenTreeCall, gtEntryPoint)
#endif
#if defined(DEBUG) || defined(INLINE_DATA)
    SHOW_FLD_OFFS(GenTreeCall, gtInlineObservation)
    SHOW_FLD_OFFS(GenTreeCall, gtRawILOffset)
#endif

    DONE_FLD_OFFS(GenTreeCall)
