...
void Compiler::fgWalkAllTreesPre(fgWalkPreFn* visitor, void* pCallBackData)
{
    for (BasicBlock* block = fgFirstBB; block != nullptr; block = block->bbNext)
    {
        ...
        {
            fgWalkTreePre(&stmt->m_rootTree, visitor, pCallBackData);
        }
    }
}
                    ...
                    if (op1->gtOper == GT_ADD)
                    {
                        ...
                        if ((tree->gtFlags & GTF_IND_ARR_INDEX) == 0)
                        {
                            ...
                            else if (varTypeIsStruct(tree))
                            {
                                ...
                                {
                                    GenTree* expr = compCurStmt->m_rootTree;
...
GenTree* Compiler::gtReplaceTree(Statement* stmt, GenTree* tree, GenTree* replacementTree)
{
    ...
    assert(treeParent != nullptr || tree == stmt->m_rootTree);
    ...
    else
    {
        ...
        else
        {
            ...
            assert(treeFirstNode == stmt->m_treeList);
            stmt->m_treeList = fgGetFirstNode(replacementTree);
        }
...
{
    fgWalkTree(&stmt->m_rootTree, fgUpdateSideEffectsPre, fgUpdateSideEffectsPost);
}
...
GenTree* Compiler::gtTryRemoveBoxUpstreamEffects(GenTree* op, BoxRemovalOptions options)
{
    ...
    GenTree* copy = copyStmt->m_rootTree;
...
GenTree* Compiler::gtOptimizeEnumHasFlag(GenTree* thisOp, GenTree* flagOp)
{
    ...
    else
    {
        ...
        thisAsgStmt->m_rootTree    = thisAsg;
    ...
    else
    {
        ...
        flagAsgStmt->m_rootTree    = flagAsg;
...
Compiler::FindLinkData Compiler::gtFindLink(Statement* stmt, GenTree* node)
{
    ...
    fgWalkResult result = fgWalkTreePre(&stmt->m_rootTree, gtFindLinkCB, &data);
