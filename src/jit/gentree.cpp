...
/*****************************************************************************
 *
 *  Given a tree, set the gtCostEx and GetCostSz() fields which
                                   ...
                                   &cns))
    {
        ...
        if (addrmodeCount > 1)
        {
            ...
            while (addrmodeCount > 0)
            {
                ...
                // addrmodeCount
                tmp->SetCosts(tmp->gtCostEx - addrmodeCount, tmp->GetCostSz() - addrmodeCount);
...
void Compiler::gtDispNode(GenTree* tree, IndentStack* indentStack, __in __in_z __in_opt const char* msg, bool isLIR)
{
    ...
    if (tree->gtSeqNum)
    {
        ...
        {
            printf("(%3u,%3u) ", tree->gtCostEx, tree->GetCostSz());
        }
        else
        {
            printf("(???"
