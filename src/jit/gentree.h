...
struct GenTreeIntCon : public GenTreeIntConCommon
{
    ...
    */
    ...
    GenTreeIntCon(var_types type, ssize_t value DEBUGARG(bool largeNode = false))
        : GenTreeIntConCommon(GT_CNS_INT, type DEBUGARG(largeNode)), gtIconVal(value), gtCompileTimeHandle(0),
...
struct GenTreeSIMD : public GenTreeOp
{
    ...
    GenTreeSIMD(var_types type, GenTreePtr op1, SIMDIntrinsicID simdIntrinsicID, var_types baseType, unsigned size)
        : GenTreeOp(GT_SIMD, type, op1, nullptr), gtSIMDIntrinsicID(simdIntrinsicID), gtSIMDBaseType(baseType),
          gtSIMDSize(size)
    {
    }
    ...
    GenTreeSIMD(var_types       type,
                ...
                var_types       baseType,
                unsigned        size)
        : GenTreeOp(GT_SIMD, type, op1, op2), gtSIMDIntrinsicID(simdIntrinsicID), gtSIMDBaseType(baseType),
          gtSIMDSize(size)
    {
    }
...
struct GenTreeIndex : public GenTreeOp
{
    ...
    GenTreeIndex(var_types type, GenTreePtr arr, GenTreePtr ind, unsigned indElemSize)
        : GenTreeOp(GT_INDEX, type, arr, ind), gtIndElemSize(indElemSize),
...
struct GenTreeBoundsChk : public GenTree
{
    ...
    GenTreeBoundsChk(genTreeOps oper, var_types type, GenTreePtr arrLen, GenTreePtr index, SpecialCodeKind kind)
        : GenTree(oper, type), gtArrLen(arrLen), gtIndex(index), gtIndRngFailBB(nullptr), gtThrowKind(kind),
          gtStkDepth(0)
    {
...
struct GenTreeArrOffs : public GenTree
{
    ...
    GenTreeArrOffs(var_types     type,
                   ...
                   unsigned char rank,
                   var_types     elemType)
        : GenTree(GT_ARR_OFFSET, type), gtOffset(offset), gtIndex(index), gtArrObj(arrObj), gtCurrDim(currDim),
          gtArrRank(rank), gtArrElemType(elemType)
    {
...
struct GenTreeStmt : public GenTree
{
    ...
    GenTreeStmt(GenTreePtr expr, IL_OFFSETX offset)
        : GenTree(GT_STMT, TYP_VOID), gtStmtExpr(expr), gtStmtList(nullptr), gtInlineContext(nullptr)
#if defined(DEBUGGING_SUPPORT) || defined(DEBUG)
          ,
...
struct GenTreePutArgStk : public GenTreeUnOp
{
    ...
    GenTreePutArgStk(genTreeOps oper,
        ...
        : GenTreeUnOp(oper, type DEBUGARG(largeNode)), gtSlotNum(slotNum), putInIncomingArgArea(_putInIncomingArgArea)
#ifdef FEATURE_UNIX_AMD64_STRUCT_PASSING
          ,
          gtPutArgStkKind(PutArgStkKindInvalid), gtNumSlots(numSlots), gtIsStruct(isStruct), gtNumberReferenceSlots(0),
          gtGcPtrs(nullptr)
    ...
    GenTreePutArgStk(genTreeOps oper,
                     ...
                     bool _putInIncomingArgArea = false DEBUGARG(GenTreePtr callNode = nullptr)
                         DEBUGARG(bool largeNode = false))
        : GenTreeUnOp(oper, type, op1 DEBUGARG(largeNode)), gtSlotNum(slotNum),
    ...
    GenTreePutArgStk(genTreeOps oper,
                     ...
                     unsigned slotNum FEATURE_UNIX_AMD64_STRUCT_PASSING_ONLY_ARG(unsigned numSlots)
                         FEATURE_UNIX_AMD64_STRUCT_PASSING_ONLY_ARG(bool isStruct) DEBUGARG(GenTreePtr callNode = NULL)
                             DEBUGARG(bool largeNode = false))
        : GenTreeUnOp(oper, type DEBUGARG(largeNode)), gtSlotNum(slotNum)
#ifdef FEATURE_UNIX_AMD64_STRUCT_PASSING
          ,
          gtPutArgStkKind(PutArgStkKindInvalid), gtNumSlots(numSlots), gtIsStruct(isStruct), gtNumberReferenceSlots(0),
          gtGcPtrs(nullptr)
