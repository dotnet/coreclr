        ...
        if (!bIntrinsicImported)
        {
            ...
            switch (callInfo->kind)
            {
                ...
                case CORINFO_CALL_CODE_POINTER:
                {
                    ...
                    GenTreePtr fptr = impLookupToTree(pResolvedToken, &callInfo->codePointerLookup, GTF_ICON_FTN_ADDR,
                                                      callInfo->hMethod);
        ...
        if (opcode == CEE_NEWOBJ)
        {
            ...
            else
            {
                ...
                if (clsFlags & CORINFO_FLG_VALUECLASS)
                {
                    ...
                    impPushOnStack(gtNewLclvNode(tmp, lvaGetRealType(tmp)),
                ...
                else
                {
                    ...
                    impPushOnStack(gtNewLclvNode(newobjThis->gtLclVarCommon.gtLclNum, TYP_REF),
    ...
    DONE:
        ...
        if (tailCall)
        {
            ...
            if (canTailCall && (tailCall & PREFIX_TAILCALL_IMPLICIT) && (verCurrentState.esStackDepth != 0))
            {
                ...
                verRaiseVerifyExceptionIfNeeded(INDEBUG("Stack should be empty after tailcall") DEBUGARG(__FILE__)
                                                    DEBUGARG(__LINE__));
#else  // _TARGET_64BIT_
                BADCODE("Stack should be empty after tailcall");
