...
#endif // DEBUG
//---------------------------------------------------------------------------
//  For a call node that returns a struct type either adjust the return type
//  to an enregisterable type, or set the flag to indicate struct return via
//  retbuf arg.
//
//  Arguments:
                                                             ...
                                                             CORINFO_CLASS_HANDLE retClsHnd)
{
    ...
    int retRegCount = retTypeDesc->GetReturnRegCount();
    if (retRegCount != -1)
    {
        if (retRegCount == 1)
        {
            // struct returned in a single register 
            callNode->gtReturnType = retTypeDesc->GetFirstReturnRegType();
        }
        else
        {
            ...
            if ((!callNode->CanTailCall()) && (!callNode->IsInlineCandidate()))
            {
                ...
                // 1. It is a tail call or 
                // 2. The call is marked for in-lining later
                return impAssignStructClassToVar(call, retClsHnd);
            }
        }
    }
...
GenTreePtr          Compiler::impFixupStructReturnType(GenTreePtr op, CORINFO_CLASS_HANDLE retClsHnd)
{
    ...
    if (varTypeIsStruct(info.compRetNativeType) && IsMultiRegVal(retClsHnd))
    {
        ...
        {       
            ...
            */
            ...
            else
            {
                ...
                if (varTypeIsStruct(op1))
                {
                    if (IsMultiRegVal(resolvedToken.hClass))
                    {
