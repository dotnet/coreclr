                                       ...
                                       ) 
{
    ...
    {
        assert(src->gtOp.gtOp2->gtType == TYP_STRUCT || src->gtOp.gtOp2->gtType == TYP_BYREF);  // Second thing is the struct or it's address.
        if (pAfterStmt)
        {
                                             ...
                                             IL_OFFSETX     ilOffset)
{
    ...
    // For CORECLR on Unix there is no varargs.
    // If the importer finds this, just bail out.
#if !FEATURE_VARARG_SUPPORTED
    if ((sig->callConv & CORINFO_CALLCONV_MASK) == CORINFO_CALLCONV_VARARG ||
        (sig->callConv & CORINFO_CALLCONV_MASK) == CORINFO_CALLCONV_NATIVEVARARG)
    {
        BADCODE("Varargs not supported in CoreCLR on Unix.");
    }
                                                         ...
                                                         CORINFO_CLASS_HANDLE retClsHnd)
{
    ...
    if (structDesc.passedInRegisters)
    {
        ...
        {
            call->gtCall.gtReturnType = impGetEightByteType(structDesc, 0);
        }
        else
        {
...
REDO_RETURN_NODE:
    ...
    else if (op->gtOper == GT_CALL)
    {
        ...
        else
        {
            ...
            if (op->gtType == TYP_STRUCT)
            {
                ...
                assert(impGetEightByteType(structDesc, 0) == op->gtCall.gtReturnType);
            }
            else
#else // !FEATURE_UNIX_AMD64_STRUCT_PASSING
            {
    ...
    if (op->gtType == TYP_STRUCT)
    {
        ...
        eeGetSystemVAmd64PassStructInRegisterDescriptor(retClsHnd, &structDesc);
        assert(structDesc.eightByteCount < CLR_SYSTEMV_MAX_EIGHTBYTES_COUNT_TO_PASS_IN_REGISTERS);
        op->gtType = impGetEightByteType(structDesc, 0);
    }
    else
#endif // FEATURE_UNIX_AMD64_STRUCT_PASSING
    {
            ...
            else
            {
                ...
                if (op1->gtType == TYP_STRUCT)
                {
                    ...
                    if (structDesc.passedInRegisters && structDesc.eightByteCount == CLR_SYSTEMV_MAX_EIGHTBYTES_COUNT_TO_PASS_IN_REGISTERS)
                    {
                        // Unbox nullable helper returns a TYP_STRUCT.
                        // We need to spill it to a temp so than can take the address of it.
                        // We need the temp so we can pass it's address to the unbox_nullable jit helper function.
...
#ifdef FEATURE_UNIX_AMD64_STRUCT_PASSING
// impGetEightByteType:
//   Returns the type of an eightbyte at index"eightByteIndex" in the structDesc specification. 
//
// args:
//   structDesc: the struct description structure.
//   eightByteIndex: the index of the eightbyte which type is needed.
//   

var_types Compiler::impGetEightByteType(const SYSTEMV_AMD64_CORINFO_STRUCT_REG_PASSING_DESCRIPTOR& structDesc, unsigned eightByteIndex)
{
    assert(eightByteIndex < structDesc.eightByteCount);
    switch (structDesc.eightByteClassifications[eightByteIndex])
    {
    case SystemVClassificationTypeInteger:
        // See typelist.h for jit type definition. 
        // All the types of size < 4 bytes are of jit type TYP_INT.
        if (structDesc.eightByteSizes[eightByteIndex] <= 4)
        {
            return TYP_INT;
        }
        else if (structDesc.eightByteSizes[eightByteIndex] <= 8)
        {
            return TYP_LONG;
        }
        else
    ...
    case SystemVClassificationTypeIntegerReference:
        assert(structDesc.eightByteSizes[eightByteIndex] == REGSIZE_BYTES);
        return TYP_REF;
        break;
    case SystemVClassificationTypeSSE:
        ...
        {
            assert(false && "Bad float type."); // Not possible.
        }
        break;
    default:
