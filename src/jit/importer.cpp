...
GenTree* Compiler::impInitializeArrayIntrinsic(CORINFO_SIG_INFO* sig)
{
    ...
    //
    GenTree* arrayAssignment = impLastStmt->GetRootTree();
    ...
    if (verCurrentState.esStackDepth != 0)
    {
        ...
        switch (block->bbJumpKind)
        {
            case BBJ_COND:
                ...
                assert(addStmt->GetRootTree()->gtOper == GT_JTRUE);
            ...
            case BBJ_SWITCH:
                ...
                addStmt = impExtractLastStmt();
                assert(addStmt->GetRootTree()->gtOper == GT_SWITCH);
        ...
        for (level = 0, tempNum = baseTmp; level < verCurrentState.esStackDepth; level++, tempNum++)
        {
            ...
            */
            ...
            if (addStmt != nullptr && !newTemps && gtHasRef(addStmt->GetRootTree(), tempNum, false))
            {
                GenTree* addTree = addStmt->GetRootTree();
