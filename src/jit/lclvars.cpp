...
int __cdecl Compiler::WtdRefCntCmpSimpleGC(const void* op1, const void* op2)
{
    ...
    if (!dsc1->lvTracked || !dsc2->lvTracked)
    {
        /* One or both untracked; sort tracked to earlier spot */
        if (dsc1->lvTracked)
            return -1;
        if (dsc2->lvTracked)
            return +1;

        // Neither variable is tracked; separate GC from non-GC
        if (varTypeIsGC(dsc1->TypeGet()))
        {
            if (!varTypeIsGC(dsc2->TypeGet()))
                return -1;
...
void Compiler::lvaSortOnly()
{
    ...
    int (__cdecl *compareFN)(const void* op1, const void* op2) = WtdRefCntCmp;
#if defined(_TARGET_AMD64_) && defined(FEATURE_CORECLR)
    if (opts.MinOpts() && !JitConfig.JitMinOptsTrackGCrefs())
        compareFN = WtdRefCntCmpSimpleGC;
    else
#endif
    if (compCodeOpt() == SMALL_CODE)
        compareFN = RefCntCmp;

    qsort(lvaRefSorted, lvaCount, sizeof(*lvaRefSorted), compareFN);
