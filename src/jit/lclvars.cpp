    ...
    */
    ...
    if (!hasRetBuffArg && (info.compRetNativeType == TYP_STRUCT))
    {
...
#endif
        {
            ...
            {
                info.compRetNativeType = impGetEightByteType(structDesc, 0);
            }
         ...
         i++, varDscInfo->varNum++, varDscInfo->varDsc++, argLst = info.compCompHnd->getArgNext(argLst))
    {
        ...
        if (canPassArgInRegisters) 
        {
            ...
            if  (verbose)
            {
                ...
                bool isFloat = false;
#if defined(FEATURE_UNIX_AMD64_STRUCT_PASSING)
                bool isFloatSecondEightByte = false;
                if ((argType == TYP_STRUCT) && (structDesc.eightByteCount >= 2))
                {
                    isFloat = varTypeIsFloating(firstEightByteType);
                    if (secondEightByteType != TYP_UNDEF)
                    {
                        isFloatSecondEightByte = varTypeIsFloating(secondEightByteType);
                    }
                }
                else
                ...
                if (argType == TYP_STRUCT)
                {
                    ...
                    {
                        printf(", secondEightByte: %s", getRegName(genMapRegArgNumToRegNum(secondAllocatedRegArgNum, secondEightByteType), isFloatSecondEightByte));
                    }
                }
                else
#endif // defined(FEATURE_UNIX_AMD64_STRUCT_PASSING)
        ...
        // TARET_POINTER_SIZE. The stack allocated space should always be multiple of TARGET_POINTER_SIZE, so roud it up.
