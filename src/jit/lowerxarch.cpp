...
//   Sets the srcCount and dstCount for all the trees without special handling FOR THESE. 
//
// args:
...
void Lowering::TreeNodeInfoInit(GenTree* stmt)
{
    ...
    while (tree)
    {
        ...
        case GT_CALL:
        {
            ...
            //  No varargs for CORECLR on Unix. Look below this flag matters only if varargs is supported.
            ...
            for (GenTreePtr list = tree->gtCall.gtCallLateArgs; list; list = list->MoveNext())
            {
                ...
                if (argNode->TypeGet() == TYP_STRUCT FEATURE_UNIX_AMD64_STRUCT_PASSING_ONLY(|| curArgTabEntry->isStruct))
                {
...
#if FEATURE_VARARG_SUPPORTED
            //  No varargs for CORECLR on Unix.
