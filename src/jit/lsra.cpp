...
{
#ifdef DEBUG
    // Early out if specifically asked not to allocate a busy reg to
    // a reg optional ref position.
    if (allocationOptional && regOptionalNoBusyRegAlloc())
    {
        return REG_NA;
    }
#endif
    ...
    {
        // This can happen only if allocating a reg is optional.
        assert(allocationOptional);
        foundReg = REG_NA;
...
LinearScan::allocateRegisters()
{
    ...
    for (auto& refPosition : refPositions)
    {
            ...
            if (currentInterval->isSpecialPutArg)
            {
                ...
                if (refType == RefTypeDef)
                {
                    ...
                    {                        
                        ...
                        // instead of to putarg_reg's interval.  So if a spill of reg allocated
                        // to source lcl var happens before its use at call node, it will
                        // lead to spill of lcl var instead of putarg_reg since phyiscal
                        // reg record is point to lcl var's interval. The assumption here is
                        // that source lcl var doesn't get spilled and re-allocated prior
                        // to its use at the call node.  This is ensured by marking
                        // physical reg record as busy until next kill. 
        ...
        if (assignedRegister == REG_NA)
        {
            ...
            if (currentRefPosition->RequiresRegOptionally() &&
                currentRefPosition->lastUse &&
            ...
            if (assignedRegister == REG_NA)
            {
                ...
                if (currentRefPosition->RequiresRegister())
                {
                    if (allocateReg)
                    {
                        ...
                        assignedRegister = allocateBusyReg(currentInterval, currentRefPosition, currentRefPosition->RequiresRegOptionally());
                    }
                    ...
                    else 
                    {
                        ...
                        noway_assert(currentRefPosition->RequiresRegOptionally());
...
LinearScan::resolveLocalRef(GenTreePtr treeNode, RefPosition * currentRefPosition)
{
    ...
    if (currentRefPosition->registerAssignment == RBM_NONE)
    {
        assert(!currentRefPosition->RequiresRegister() ||
               currentRefPosition->RequiresRegOptionally());
        ...
        if (currentRefPosition->refType != RefTypeBB)
        {
            ...
            if (interval != nullptr)
            {
                ...
                if (currentRefPosition->spillAfter || currentRefPosition->lastUse)
                {
                    ...
                    {
                        assert(currentRefPosition->RequiresRegOptionally());
                    }
                }
            }
        }
