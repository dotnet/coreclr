                                     ...
                                     LsraLocation currentLoc)
{
...
#ifdef FEATURE_SIMD
        if (tree->OperIsLocalStore() && !useNode->IsMultiRegCall() && varDefInterval == nullptr)
        {
    ...
    {        
        ...
        if (!noPush)
        {
            stack->Push(LocationInfo(lastDefLocation, interval, tree, (unsigned) i));
        }
...
{  
    ...
    // Because multi-reg call node is the only node whose RefTypeDef positions get
    // independently spilled or reloaded.  It is possible that one of its RefTypeDef
    // position got spilled and the next use of it requires it to be in a different
    // register.
    //
