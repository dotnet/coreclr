    ...
    if (inliningFailed)
    {
        if (call->gtReturnType != TYP_VOID)
        {
            JITDUMP("Inlining [%06u] failed, so bashing [%06u] to NOP\n", dspTreeID(call),
                    dspTreeID(fgMorphStmt->m_rootTree));
            ...
            noway_assert(fgMorphStmt->m_rootTree == call);
            fgMorphStmt->m_rootTree = gtNewNothingNode();
        }
    }
}
...
void Compiler::fgMorphCallInlineHelper(GenTreeCall* call, InlineResult* result)
{
    ...
    */
    ...
    if (verbose)
    {
        ...
        printTreeID(fgMorphStmt->m_rootTree);
        printf(" in " FMT_BB ":\n", compCurBB->bbNum);
        gtDispTree(fgMorphStmt->m_rootTree);
...
GenTree* Compiler::fgMorphPotentialTailCall(GenTreeCall* call)
{
    ...
    GenTree* stmtExpr = fgMorphStmt->m_rootTree;
...
void Compiler::fgMorphRecursiveFastTailCallIntoLoop(BasicBlock* block, GenTreeCall* recursiveTailCall)
{
    ...
    assert(recursiveTailCall == lastStmt->m_rootTree);

    // Transform recursive tail call into a loop.

    Statement* earlyArgInsertionPoint = lastStmt;
    IL_OFFSETX callILOffset           = lastStmt->m_ILoffsx;
    ...
    {
        Statement* thisArgStmt = fgNewStmt(thisArg, callILOffset);
    ...
    for (GenTreeCall::Use& use : recursiveTailCall->Args())
    {
        ...
        if (!earlyArg->IsNothingNode() && !earlyArg->IsArgPlaceHolderNode())
        {
            if ((earlyArg->gtFlags & GTF_LATE_ARG) != 0)
            {
                ...
                Statement* earlyArgStmt = fgNewStmt(earlyArg, callILOffset);
    ...
    if (!info.compIsStatic && (lvaArg0Var != info.compThisArg))
    {
        ...
        Statement* arg0AssignmentStmt = fgNewStmt(arg0Assignment, callILOffset);
        ...
        for (varNum = 0, varDsc = lvaTable; varNum < lvaCount; varNum++, varDsc++)
        {
            ...
            if (!varDsc->lvIsParam)
            {
                ...
                if (isUserLocal || structWithGCFields)
                {
                    ...
                    Statement* initStmt = fgNewStmt(init, callILOffset);
                                                           ...
                                                           Statement*     paramAssignmentInsertionPoint)
{
    ...
    if (needToAssignParameter)
    {
        if (argInTemp == nullptr)
        {
            ...
            Statement* tmpAssignStmt = fgNewStmt(tmpAssignNode, callILOffset);
        ...
        paramAssignStmt          = fgNewStmt(paramAssignNode, callILOffset);
...
GenTree* Compiler::fgMorphCall(GenTreeCall* call)
{
    ...
    if (call->CanTailCall())
    {
        ...
        if (fgGlobalMorph && call->HasMultiRegRetVal())
        {
            ...
            Statement* assgStmt = fgNewStmt(assg, compCurStmt->m_ILoffsx);
            ...
            if (verbose)
            {
                ...
                gtDispTree(assgStmt->m_rootTree);
            }
...
bool Compiler::fgFoldConditional(BasicBlock* block)
{
    ...
    if (block->bbJumpKind == BBJ_COND)
    {
        ...
        if (lastStmt->m_rootTree->gtOper == GT_CALL)
        {
        ...
        noway_assert(lastStmt->m_rootTree->gtOper == GT_JTRUE);

        /* Did we fold the conditional */

        noway_assert(lastStmt->m_rootTree->gtOp.gtOp1);
        GenTree* cond;
        cond = lastStmt->m_rootTree->gtOp.gtOp1;
    ...
    else if (block->bbJumpKind == BBJ_SWITCH)
    {
        ...
        noway_assert(lastStmt->m_rootTree->gtOper == GT_SWITCH);
...
bool Compiler::fgMorphBlockStmt(BasicBlock* block, Statement* stmt DEBUGARG(const char* msg))
{
    ...
    GenTree* morph = fgMorphTree(stmt->m_rootTree);
...
void Compiler::fgMorphStmts(BasicBlock* block, bool* lnot, bool* loadw)
{
    ...
    *lnot = *loadw = false;
    ...
    for (; stmt != nullptr; prev = stmt->m_rootTree, stmt = stmt->m_next)
    {
        if (fgRemoveRestOfBlock)
        {
            fgRemoveStmt(block, stmt);
...
#ifdef FEATURE_SIMD
        if (opts.OptimizationEnabled() && stmt->m_rootTree->TypeGet() == TYP_FLOAT &&
            stmt->m_rootTree->OperGet() == GT_ASG)
        {
    ...
    if (fgRemoveRestOfBlock)
    {
        if ((block->bbJumpKind == BBJ_COND) || (block->bbJumpKind == BBJ_SWITCH))
        {
            ...
            GenTree* last = lastStmt->m_rootTree;
                ...
                ((block->bbJumpKind == BBJ_SWITCH) && (last->gtOper == GT_SWITCH)))
            {
                ...
                lastStmt->m_rootTree = fgMorphTree(op1);
            }
        }
    ...
    do
    {
        ...
        if (block->bbJumpKind == BBJ_RETURN)
        {
            if ((genReturnBB != nullptr) && (genReturnBB != block) && ((block->bbFlags & BBF_HAS_JMP) == 0))
            {
                ...
                Statement* lastStmt = block->lastStmt();
                GenTree*   ret      = (lastStmt != nullptr) ? lastStmt->m_rootTree : nullptr;
                ...
                else
                {
                    ...
                    if (genReturnLocal != BAD_VAR_NUM)
                    {
                        ...
                        IL_OFFSETX offset          = lastStmt->m_ILoffsx;
                        GenTree*   tree =
...
void Compiler::fgExpandQmarkForCastInstOf(BasicBlock* block, Statement* stmt)
{
    ...
    Statement* trueStmt = fgNewStmtFromTree(trueExpr, stmt->m_ILoffsx);
    ...
    Statement* helperStmt = fgNewStmtFromTree(helperExpr, stmt->m_ILoffsx);
...
void Compiler::fgExpandQmarkStmt(BasicBlock* block, Statement* stmt)
{
    ...
    if (hasFalseExpr)
    {
        ...
        Statement* falseStmt = fgNewStmtFromTree(falseExpr, stmt->m_ILoffsx);
...
public:
    ...
    void VisitStmt(Statement* stmt)
    {
        ...
        if (m_compiler->verbose)
        {
            ...
            m_compiler->gtDispTree(stmt->m_rootTree);
...
bool Compiler::fgMorphCombineSIMDFieldAssignments(BasicBlock* block, Statement* stmt)
{
    ...
    {
        GenTree* exp = curStmt->m_rootTree;
    ...
    if (verbose)
    {
        ...
        printTreeID(stmt->m_rootTree);
        printf(" to stmt");
        printTreeID(lastStmt->m_rootTree);
        printf("\n");
    }
#endif
...
bool Compiler::fgCheckStmtAfterTailCall()
{
    ...
    // that are supported.
    GenTree* callExpr = callStmt->m_rootTree;
    ...
    if (callExpr->gtOper != GT_RETURN)
    {
        ...
        if (nextMorphStmt != nullptr && nextMorphStmt->m_rootTree->gtOper != GT_RETURN)
        {
            ...
            GenTree* popExpr          = popStmt->m_rootTree;
    ...
    if (nextMorphStmt != nullptr)
    {
        ...
        if (callExpr->gtOper != GT_ASG)
        {
            ...
            GenTree*   retExpr = retStmt->m_rootTree;
        ...
        else
        {
            ...
            if (nextMorphStmt->m_rootTree->gtOper == GT_ASG)
            {
                Statement* moveStmt = nextMorphStmt;
                GenTree*   moveExpr = nextMorphStmt->m_rootTree;
            ...
            {
                Statement* retStmt = nextMorphStmt;
                GenTree*   retExpr = nextMorphStmt->m_rootTree;
                noway_assert(retExpr->gtOper == GT_RETURN);
