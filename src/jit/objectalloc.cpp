...
void ObjectAllocator::MarkEscapingVarsAndBuildConnGraph()
{
    ...
    foreach_block(comp, block)
    {
        for (Statement* stmt : block->Statements())
        {
            ...
            buildConnGraphVisitor.WalkTree(stmt->GetRootTreePointer(), nullptr);
        }
    }
}
...
bool ObjectAllocator::MorphAllocObjNodes()
{
    ...
    foreach_block(comp, block)
    {
        ...
        for (Statement* stmt : block->Statements())
        {
            GenTree* stmtExpr = stmt->GetRootTree();
            GenTree* op2      = nullptr;
            ...
            else
            {
                ...
                // canonical form.
                comp->fgWalkTreePre(stmt->GetRootTreePointer(), AssertWhenAllocObjFoundVisitor);
            }
#endif // DEBUG
        }
    }
...
void ObjectAllocator::RewriteUses()
{
    ...
    foreach_block(comp, block)
    {
        for (Statement* stmt : block->Statements())
        {
            ...
            rewriteUsesVisitor.WalkTree(stmt->GetRootTreePointer(), nullptr);
        }
    }
}
