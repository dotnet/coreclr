...
{
    GenTree* test = testStmt->m_rootTree;
        ...
        opr2->IsIntegralConst(0))
    {
        ...
        Statement* prevStmt = testStmt->m_prev;
        if (prevStmt == nullptr)
        {
            return false;
        }

        GenTree* tree = prevStmt->m_rootTree;
    ...
    BasicBlock* head, BasicBlock* bottom, BasicBlock* top, GenTree** ppInit, GenTree** ppTest, GenTree** ppIncr)
{
    ...
    Statement* incrStmt = testStmt->m_prev;
    if (incrStmt == nullptr || optIsLoopIncrTree(incrStmt->m_rootTree) == BAD_VAR_NUM)
    {
        ...
        if (optIsLoopIncrTree(toplastStmt->m_rootTree) != BAD_VAR_NUM)
        {
    ...
    if (initStmt->m_compilerAdded)
    {
        ...
        else
        {
            ...
            noway_assert(initStmt->m_rootTree->gtOper == GT_JTRUE);
        }
#endif // DEBUG
        if (doGetPrev)
        {
            initStmt = initStmt->m_prev;
        }
        noway_assert(initStmt != nullptr);
    }

    *ppInit = initStmt->m_rootTree;
    *ppTest = testStmt->m_rootTree;
    *ppIncr = incrStmt->m_rootTree;
...
void Compiler::optUnrollLoops()
{
    ...
    for (unsigned lnum = optLoopCount - 1; lnum != ~0U; --lnum)
    {
        ...
        GenTree* incr = incrStmt->m_rootTree;
        ...
        GenTree* init = initStmt->m_rootTree;
        GenTree* test = testStmt->m_rootTree;
        ...
        if ((init->gtOper != GT_ASG) || (init->gtOp.gtOp1->gtOper != GT_LCL_VAR) ||
            ...
            (testStmt->m_rootTree->gtOper != GT_JTRUE))
        {
        ...
        /* Create the unrolled loop statement list */
        {
            ...
            for (lval = lbeg; totalIter; totalIter--)
            {
                for (block = head->bbNext;; block = block->bbNext)
                {
                    ...
                    if (block == bottom)
                    {
                        ...
                        GenTree*   testCopyExpr = testCopyStmt->m_rootTree;
                        ...
                        {
                            testCopyStmt->m_rootTree = sideEffList;
                        }
            ...
            if (head->bbJumpKind == BBJ_COND)
            {
                ...
                testStmt = preHeaderStmt->m_prev;

                noway_assert((testStmt != nullptr) && (testStmt->m_next == nullptr));
                noway_assert(testStmt->m_rootTree->gtOper == GT_JTRUE);

                initStmt = testStmt->m_prev;
            ...
            if (verbose)
            {
                ...
                gtDispTree(initStmt->m_rootTree);
                printf("\n");
...
static Statement* optFindLoopTermTest(BasicBlock* bottom)
{
    ...
    Statement* result = testStmt->m_prev;

#ifdef DEBUG
    while (testStmt->m_next != nullptr)
    {
        testStmt = testStmt->m_next;
    }
...
void Compiler::fgOptWhileLoop(BasicBlock* block)
{
    ...
    GenTree* condTree = condStmt->m_rootTree;
    ...
    copyOfCondStmt->m_compilerAdded = true;

    if (opts.compDbgInfo)
    {
        copyOfCondStmt->m_ILoffsx = condStmt->m_ILoffsx;
    }
    ...
    if (verbose)
    {
        ...
        gtDispTree(copyOfCondStmt->m_rootTree);
    }
...
int Compiler::optIsSetAssgLoop(unsigned lnum, ALLVARSET_VALARG_TP vars, varRefKinds inds)
{
    ...
    if (!(loop->lpFlags & LPFLG_ASGVARS_YES))
    {
        ...
        for (/**/; /**/; beg = beg->bbNext)
        {
            ...
            for (Statement* stmt = beg->FirstNonPhiDef(); stmt != nullptr; stmt = stmt->m_next)
            {
                fgWalkTreePre(&stmt->m_rootTree, optIsVarAssgCB, &desc);
...
void Compiler::optPerformHoistExpr(GenTree* origExpr, unsigned lnum)
{
    ...
    Statement* hoistStmt       = fgNewStmt(hoist);
...
void Compiler::optHoistLoopBlocks(unsigned loopNum, ArrayStack<BasicBlock*>* blocks, LoopHoistContext* hoistContext)
{
    ...
    public:
        ...
        void HoistBlock(BasicBlock* block)
        {
            ...
            {
                WalkTree(&stmt->m_rootTree, nullptr);
                assert(m_valueStack.TopRef().Node() == stmt->m_rootTree);

                if (m_valueStack.TopRef().m_hoistable)
                {
                    m_compiler->optHoistCandidate(stmt->m_rootTree, m_loopNum, m_hoistContext);
                }
...
bool Compiler::optIdentifyLoopOptInfo(unsigned loopNum, LoopCloneContext* context)
{
    ...
    for (BasicBlock* block = beg; block != end->bbNext; block = block->bbNext)
    {
        ...
        for (Statement* stmt : block->Statements())
        {
            ...
            fgWalkTreePre(&stmt->m_rootTree, optCanOptimizeByLoopCloningVisitor, &info, lclVarsOnly, computeStack);
        }
    }
...
void Compiler::optOptimizeBoolsGcStress(BasicBlock* condBlock)
{
    ...
    GenTree* cond = condBlock->lastStmt()->m_rootTree;
