...
bool Compiler::optIsLoopTestEvalIntoTemp(GenTreeStmt* testStmt, GenTreeStmt** newTestStmt)
{
        ...
        opr2->IsIntegralConst(0))
    {
        ...
        GenTreeStmt* prevStmt = testStmt->gtPrevStmt;
        if (prevStmt == nullptr)
        {
            return false;
        }
    ...
    BasicBlock* head, BasicBlock* bottom, BasicBlock* top, GenTree** ppInit, GenTree** ppTest, GenTree** ppIncr)
{
    ...
    GenTreeStmt* lastStmt = bottom->firstStmt()->gtPrevStmt;
    noway_assert(lastStmt != nullptr && lastStmt->gtNextStmt == nullptr);

    GenTreeStmt* newTestStmt;
    if (optIsLoopTestEvalIntoTemp(lastStmt, &newTestStmt))
    {
        lastStmt = newTestStmt;
    }

    // Check if we have the incr tree before the test tree, if we don't,
    // check if incr is part of the loop "top".
    GenTreeStmt* incrStmt = lastStmt->gtPrevStmt;
    if (incrStmt == nullptr || optIsLoopIncrTree(incrStmt->gtStmtExpr) == BAD_VAR_NUM)
    {
        ...
        // If the prev stmt to loop test is not incrStmt, then check if we have loop test evaluated into a tmp.
        GenTreeStmt* topLast = top->firstStmt()->gtPrevStmt;
        if (optIsLoopIncrTree(topLast->gtStmtExpr) != BAD_VAR_NUM)
        {
            incrStmt = topLast;
        }
        else
    ...
    GenTreeStmt* initStmt = phdrStmt->gtPrevStmt;
    noway_assert(initStmt != nullptr && (initStmt->gtNextStmt == nullptr));
    ...
    if (initStmt->gtFlags & GTF_STMT_CMPADD)
    {
        ...
        if (opts.optRepeat)
        {
            ...
            doGetPrev = (initStmt->gtPrevStmt != nullptr);
        }
        else
        {
...
{
    GenTreeStmt* testt = bottom->firstStmt();

    assert(testt != nullptr);

    GenTreeStmt* result = testt->gtPrevStmt;

#ifdef DEBUG
    while (testt->gtNextStmt != nullptr)
    {
        testt = testt->gtNextStmt;
    }
...
void Compiler::fgCreateLoopPreHeader(unsigned lnum)
{
    ...
    for (GenTreeStmt* stmt = top->firstStmt(); stmt != nullptr; stmt = stmt->gtNextStmt)
    {
...
void Compiler::optComputeLoopSideEffectsOfBlock(BasicBlock* blk)
{
    ...
    for (GenTreeStmt* stmt = blk->FirstNonPhiDef(); stmt != nullptr; stmt = stmt->gtNextStmt)
    {
...
void Compiler::optOptimizeBoolsGcStress(BasicBlock* condBlock)
{
    ...
    GenTree* cond = condBlock->firstStmt()->gtPrevStmt->gtStmtExpr;
...
void Compiler::optOptimizeBools()
{
    ...
    do
    {
        ...
        for (BasicBlock* b1 = fgFirstBB; b1; b1 = b1->bbNext)
        {
            ...
            GenTreeStmt* s1 = b1->firstStmt()->gtPrevStmt;
