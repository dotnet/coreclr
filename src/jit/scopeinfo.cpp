        ...
        if (lclVarDsc1->lvIsRegArg)
        {
            ...
            if (lclVarDsc1->TypeGet() == TYP_STRUCT)
            {
                ...
                for (unsigned nCnt = 0; nCnt < structDesc.eightByteCount; nCnt++)
                {
                    ...
                    switch (structDesc.eightByteClassifications[nCnt])
                    {
                    case SystemVClassificationTypeInteger:
                        if (structDesc.eightByteSizes[nCnt] < 8)
                        {
                            regType = TYP_INT;
                        }
                        else if (structDesc.eightByteSizes[nCnt] == 8)
                        {
                            regType = TYP_LONG;
                        }
                        break;
                    ...
                    case SystemVClassificationTypeSSE:
                        if (structDesc.eightByteSizes[nCnt] == 4)
                        {
                            regType = TYP_FLOAT;
                        }
                        else if (structDesc.eightByteSizes[nCnt] == 8)
