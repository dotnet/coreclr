                ...
                for (GenTreeStmt* stmt = handler->firstStmt(); stmt != nullptr; stmt = stmt->getNextStmt())
                {
                    ...
                    if (tree->AsOp()->gtOp1->gtLclVar.GetLclNum() == lclNum)
                    {
                        ...
                        for (GenTreeArgList* curArgs = args; curArgs != nullptr; curArgs = curArgs->Rest())
                        {
                            ...
                            assert(phiArg->GetSsaNum() != ssaNum);
                        }
#endif
...
void SsaBuilder::AssignPhiNodeRhsVariables(BasicBlock* block, SsaRenameState* pRenameState)
{
    for (BasicBlock* succ : block->GetAllSuccs(m_pCompiler))
    {
             ...
             stmt              = stmt->getNextStmt())
        {
            ...
            GenTreeArgList* argList = (phiNode->AsOp()->gtOp1 == nullptr ? nullptr : phiNode->AsOp()->gtOp1->AsArgList());
            bool            found   = false;
            ...
            {
                GenTree* newPhiArg =
                    new (m_pCompiler, GT_PHI_ARG) GenTreePhiArg(tree->AsOp()->gtOp1->TypeGet(), lclNum, ssaNum, block);
...
void SsaBuilder::Print(BasicBlock** postOrder, int count)
{
    for (int i = count - 1; i >= 0; --i)
    {
        ...
        m_pCompiler->gtDispTreeList(postOrder[i]->getBBTreeList());
    }
}
#endif // DEBUG
