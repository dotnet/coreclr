...
namespace System
{
    internal static class DateTimeParse
    {
        ...
        internal static bool TryParseExact(ReadOnlySpan<char> s, ReadOnlySpan<char> format, DateTimeFormatInfo dtfi, DateTimeStyles style, ref DateTimeResult result)
        {
            if (s.Length == 0)
            {
                result.SetFailure(ParseFailureKind.FormatWithParameter, nameof(SR.Format_BadDateTime), string.Empty);
                return false;
            }

            if (format.Length == 0)
            {
                result.SetFailure(ParseFailureKind.FormatWithParameter, nameof(SR.Format_BadFormatSpecifier), string.Empty);
                return false;
            }
                                                ...
                                                DateTimeFormatInfo dtfi, DateTimeStyles style, ref DateTimeResult result)
        {
            ...
            result.SetFailure(ParseFailureKind.FormatWithOriginalDateTime, nameof(SR.Format_BadDateTime));
            return (false);
        }
        ...
        **      FormatException if invalid timezone format is found.
        ...
        private static Boolean Lex(DS dps, ref __DTString str, ref DateTimeToken dtok, ref DateTimeRawInfo raw, ref DateTimeResult result, ref DateTimeFormatInfo dtfi, DateTimeStyles styles)
        {
            ...
            switch (tokenType)
            {
                ...
                case TokenType.UnknownToken:
                    ...
                    {
                        result.SetFailure(ParseFailureKind.FormatWithOriginalDateTimeAndParameter, nameof(SR.Format_UnknowDateTimeWord), str.Index);
        ...
        private static Boolean DetermineTimeZoneAdjustments(ref __DTString str, ref DateTimeResult result, DateTimeStyles styles, Boolean bTimeOnly)
        {
            ...
            else
            {
                ...
                {
                    result.SetFailure(ParseFailureKind.Format, nameof(SR.Format_OffsetOutOfRange));
                    return false;
                }
            }
        ...
        private static Boolean DateTimeOffsetTimeZonePostProcessing(ref __DTString str, ref DateTimeResult result, DateTimeStyles styles)
        {
            ...
            {
                result.SetFailure(ParseFailureKind.Format, nameof(SR.Format_UTCOutOfRange));
                return false;
            }
        ...
        **Exceptions:
        ...
        private static bool CheckDefaultDateTime(ref DateTimeResult result, ref Calendar cal, DateTimeStyles styles)
        {
            if ((result.flags & ParseFlags.CaptureOffset) != 0)
            {
                ...
                {
                    result.SetFailure(ParseFailureKind.Format, nameof(SR.Format_MissingIncompleteDate));
                    return false;
                }
            }
            ...
            ref DateTimeResult result)
        {
            ...
            switch (ch)
            {
                ...
                case '%':
                    ...
                    {
                        result.SetFailure(ParseFailureKind.FormatWithFormatSpecifier, nameof(SR.Format_BadFormatSpecifier));
                        result.failedFormatSpecifier = format.Value;
                        return false;
                    }
                    break;
                case '\\':
            ...
            ref DateTimeResult result)
        {
            ...
            if (formatParam.Length == 1)
            {
                if (((result.flags & ParseFlags.CaptureOffset) != 0) && formatParam[0] == 'U')
                {
                    ...
                    result.SetFailure(ParseFailureKind.FormatWithFormatSpecifier, nameof(SR.Format_BadFormatSpecifier));
                    result.failedFormatSpecifier = formatParam;
                    return false;
                }
