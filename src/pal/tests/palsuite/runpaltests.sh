...
# Create path to a folder where PAL tests will store their output. Determine the folder where output files were requested to be
# copied. First check if the output folder was passed as a parameter to the script. It is supposed be the second parameter so
...
# Remove the temporary test output directory, and the directory where output files were requested to be copied.
...
do
  ...
  # Change back to the output directory, and remove the test's working directory if it's not empty
  cd $PAL_TEST_OUTPUT_DIR
...
if [ "$COPY_TO_TEST_OUTPUT_DIR" != "$PAL_TEST_OUTPUT_DIR" ]; then
  cp -f -r $PAL_TEST_OUTPUT_DIR/* $COPY_TO_TEST_OUTPUT_DIR/
