...
def generateEventPipeCmakeFile(etwmanifest, eventpipe_directory):
    ...
    with open_for_update(os.path.join(eventpipe_directory, "CMakeLists.txt")) as topCmake:
        ...
        topCmake.write('            "%s/eventpipehelpers.cpp"\n' % (eventpipe_dirname,))
        topCmake.write("""        )
...
{
    if(!str) return true;
    unsigned int byteCount = (wcslen(str) + 1) * sizeof(*str);
...
def generateEventPipeFiles(
    ...
    # generate CMake file
    generateEventPipeCmakeFile(etwmanifest, intermediate)
