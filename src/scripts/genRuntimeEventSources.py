...
def generateEventSources(manifestFullPath, intermediatesDirFullPath):
    ...
    # Find the public provider node.
    for node in manifest.getElementsByTagName("provider"):
        if node.getAttribute("name") == "Microsoft-Windows-DotNETRuntime":
            providerNode = node
            break

    # Generate a full path to the output file and open the file for open_for_update.
    outputFilePath = os.path.join(intermediatesDirFullPath,'RuntimeEventSource.cs')
...
"""
        ...
        increaseTabLevel()
        writeOutput(outputFile, "[EventSource(Name = \"Microsoft-Windows-DotNETRuntime\", Guid = \"" + providerNode.getAttribute("guid") + "\")]\n")
        writeOutput(outputFile, "internal sealed unsafe class RuntimeEventSource : EventSource\n")
        writeOutput(outputFile, "{\n")
        increaseTabLevel()
