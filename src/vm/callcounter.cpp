...
CallCounterEntry CallCounterEntry::CreateWithStartupTierCallCountingDisabled(const MethodDesc *m)
{
    WRAPPER_NO_CONTRACT;
    _ASSERTE(m != nullptr);

    CallCounterEntry entry(m, INT_MAX);
    _ASSERTE(!entry.IsStartupTierCallCountingEnabled());
    return entry;
}
...
bool CallCounter::IsEligibleForStartupTierCallCounting(MethodDesc* pMethodDesc)
{
    WRAPPER_NO_CONTRACT;
...
bool CallCounter::IsStartupTierCallCountingEnabled(MethodDesc* pMethodDesc)
{
    WRAPPER_NO_CONTRACT;
    _ASSERTE(pMethodDesc != NULL);
    _ASSERTE(pMethodDesc->IsEligibleForTieredCompilation());
    _ASSERTE(IsEligibleForStartupTierCallCounting(pMethodDesc));

    SpinLockHolder holder(&m_lock);

    const CallCounterEntry *entry = m_methodToCallCount.LookupPtr(pMethodDesc);
    return entry == nullptr || entry->IsStartupTierCallCountingEnabled();
}

void CallCounter::DisableStartupTierCallCounting(MethodDesc* pMethodDesc)
{
    WRAPPER_NO_CONTRACT;
    _ASSERTE(pMethodDesc != NULL);
    _ASSERTE(pMethodDesc->IsEligibleForTieredCompilation());
    ...
    {
        existingEntry->DisableStartupTierCallCounting();
        return;
    }
    ...
    m_methodToCallCount.Add(CallCounterEntry::CreateWithStartupTierCallCountingDisabled(pMethodDesc));
}
    ...
    BOOL* wasPromotedToNextTierRef)
{
    ...
    bool isFirstStartupTierCall = false;
    int startupTierCallCountLimit;
    {
        ...
        if (pEntry == NULL)
        {
            isFirstStartupTierCall = true;
            startupTierCallCountLimit = (int)g_pConfig->TieredCompilation_StartupTier_CallCountThreshold() - 1;
            _ASSERTE(startupTierCallCountLimit >= 0);
            m_methodToCallCount.Add(CallCounterEntry(pMethodDesc, startupTierCallCountLimit));
        }
        else if (pEntry->IsStartupTierCallCountingEnabled())
        {
            pEntry->startupTierCallCountLimit--;
            startupTierCallCountLimit = pEntry->startupTierCallCountLimit;
        }
        else
        {
    ...
    pTieredCompilationManager->OnStartupTierMethodCalled(
        pMethodDesc,
        isFirstStartupTierCall,
        startupTierCallCountLimit,
