...
{    
    ...
    if (data.cElements != 0)
    {
        ...
        for (int i = 0; i < data.cElements; i++)
        {
            ...
            // NULL the source file name
            pStackFrameHelper->rgFilename->SetAt(iNumValidFrames, NULL);

            // NULL the assembly path string
            pStackFrameHelper->rgAssemblyPath->SetAt(iNumValidFrames, NULL);
