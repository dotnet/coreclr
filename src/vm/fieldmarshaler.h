...
public:
    ...
    START_COPY_TO_IMPL(FieldMarshaler_FixedBuffer)
    {
        pDestFieldMarshaller->m_pNestedMethodTable.SetValueMaybeNull(GetMethodTable());
        pDestFieldMarshaller->size = size;
    }
    END_COPY_TO_IMPL(FieldMarshaler_FixedBuffer)
    ...
    VOID ScalarUpdateCLRImpl(const VOID *pNative, LPVOID pCLR) const
    {
        ...
        memcpyNoGCRefs(pCLR, pNative, sizeof(size));
    }
