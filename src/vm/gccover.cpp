...
bool IsGcCoveregeInterruptInstruction(LPVOID instrPtr)
{
...
bool IsOriginalInstruction(LPVOID instrPtr, GCCoverageInfo* gcCover, DWORD offset)
{
#if defined(_TARGET_ARM64_)
    UINT32 instrVal = *reinterpret_cast<UINT32*>(instrPtr);
    DWORD origInstrVal = *(DWORD*)(gcCover->savedCode + offset);
    ...
    if (instrLen == 2)
    {
        WORD origInstrVal = *(WORD*)(gcCover->savedCode + offset);
        return (instrVal == origInstrVal);
    }
    else
    {
...
bool IsGcCoverageInterrupt(LPVOID ip)
{
    ...
    if (IsGcCoveregeInterruptInstruction(ip))
    {
        return true;
    }

    if (IsOriginalInstruction(ip, gcCover, codeInfo.GetRelOffset()))
    {
...
void DoGcStress (PCONTEXT regs, MethodDesc *pMD)
{
    ...
    if (!IsGcCoveregeInterruptInstruction((LPVOID)instrPtr))
    {
        _ASSERTE(IsOriginalInstruction((LPVOID)instrPtr, gcCover, offset));
