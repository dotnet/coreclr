...
GcInfoDecoder::GcInfoDecoder(
    ...
    UINT32 breakOffset)
    : m_Reader(dac_cast<PTR_CBYTE>(gcInfoToken.Info)), m_InstructionOffset(breakOffset), m_IsInterruptible(false), m_ReturnKind(RT_Illegal)
#ifdef _DEBUG
      ,
      m_Flags(flags), m_GcInfoAddress(dac_cast<PTR_CBYTE>(gcInfoToken.Info))
#endif
      ,
    ...
    PREGDISPLAY pRD)
{
    ...
    if (regNum > 4)
        regNum--; // rsp is skipped in Redhawk RegDisplay
#else
    ...
    void *hCallBack)
{
    ...
    LOG((LF_GCROOTS, LL_INFO1000, /* Part One */
         "Reporting %s" FMT_STK,
         ((GC_SP_REL == spBase) ? "" : ((GC_CALLER_SP_REL == spBase) ? "caller's " : ((GC_FRAMEREG_REL == spBase) ? "frame " : "<unrecognized GcStackSlotBase> "))),
         DBG_STK(spOffset)));
