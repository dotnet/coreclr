                                    ...
                                    HostCallPreference hostCallPreference /* = (HostCallPreference)(-1) */)
{
    ...
    again:
        ...
        case 0xE9: // jmp <disp32>
        {
            PTR_BYTE tmpIp = ip + ((__int32)*dac_cast<PTR_DWORD>(ip + 1) + 5);
            ip = tmpIp;
        }
        break;
        ...
        case 0x0f: // follow non-zero jumps:
            ...
            else if ((ip[1] & 0xf0) == 0x40)
            { //cmov mod/rm
                ++ip;
                datasize = 0;
                goto decodeRM;
            }
            else if (ip[1] >= 0x10 && ip[1] <= 0x17)
            { // movups, movlps, movhps, unpcklpd, unpckhpd
                ++ip;
                datasize = 0;
                goto decodeRM;
            }
        ...
        case 0x68: // push 0xXXXXXXXX
            ...
            else
            {
                ...
                if ((*ip & 0xFE) == 0xE8 && callsInstrumented())
                { // It is a call or a jump (E8 or E9)
                    PTR_BYTE tmpIp = ip + 5;
                    ...
                    { // jmp [xxxx] (to external dll)
                        ...
                        else if (shouldEnterCall(target))
                        {                       // Do logic for calls
                            epilogCallRet = ip; // remember our return address
                            --ESP;              // simulate pushing the return address
                            ip = target;
        ...
        ret_with_epilogHelperCheck:
            if (epilogCallRet != 0)
            { // we are returning from a special epilog helper
                ip = epilogCallRet;
                epilogCallRet = 0;
