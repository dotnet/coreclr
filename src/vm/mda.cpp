...
extern void PrintToStdErrW(const WCHAR *pwzString);
...
MdaEnvironment::MdaEnvironment()
{
    ...
    if (m_bDisable == FALSE)
    {
        ...
        PathString szExe;
        if (!WszGetModuleFileName(NULL, szExe))
            return;
...
void MdaXmlMessage::SendDebugEvent()
{
    ...
    EX_TRY
    {
        ...
        StackSString message = m_localizedMessage;
        message.Append(W('\n'));
...
void MdaXPath::Find(SArray<MdaXPathVariable>& args, SString* pWildCard, va_list argItr)
{
    ...
    for (COUNT_T i = 0; i < GetArgCount(); i ++)
    {
        ...
        if (varType == XPathVarElemDeclDef)
            args[i].m_u.m_elemDeclDef = (MdaElemDeclDef) va_arg(argItr, INT32);

        else if (varType == XPathVarAttrDeclDef)
            args[i].m_u.m_attrDeclDef = (MdaAttrDeclDef) va_arg(argItr, INT32);
