...
LPVOID NDirectMethodDesc::FindEntryPoint(HINSTANCE hMod) const
{
    ...
    // The symbol name and version are stored in the EntryPoint as <name>@<version>
    // Using '@' as a separator is okay, since symbol names generated by gcc/clang
    // (described in the c++ itanium abi) don't contain the '@' character.
    // The lookup can be skipped by setting DllImportAttribute.ExactSpelling
    if (!IsNativeNoMangled())
    {
        int atPosition = 0;
        int entrypointLength = 0;
        LPCUTF8 pszEntrypointName = GetEntrypointName();
        while (pszEntrypointName[entrypointLength] != '\0')
        {
            if (pszEntrypointName[entrypointLength] == '@')
            {
                atPosition = entrypointLength;
            }
            entrypointLength++;
        }
        if (pszEntrypointName[atPosition] == '@')
        {
            LPSTR szSymbol = ((LPSTR)_alloca(entrypointLength + 1));
            strcpy_s(szSymbol, entrypointLength + 1, pszEntrypointName);
            szSymbol[atPosition] = '\0';
            LPSTR szVersion = &szSymbol[atPosition + 1];

            pFunc = GetVersionedProcAddress(hMod, szSymbol, szVersion);
            if (pFunc != NULL)
            {
                return (LPVOID)pFunc;
            }
