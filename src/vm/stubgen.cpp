...
void ILCodeStream::Emit(ILInstrEnum instr, INT16 iStackDelta, UINT_PTR uArg)
{
    ...
    {
        // In R2R generation, some stubs may be generated. This stub generation process isn't safe for all IL instructions
        // so instead of predicting which stubs might be valid to generate, the process is to generate stubs that use
        // a whitelist of instructions, and when generating an instruction that isn't always safe to generate, produce an
        // exception that will cancel generation of that code. For uses of generally banned instructions that are acceptable
        // in context, the higher level component may use the OverrideNonR2RSafeILStubChecksHolder to allow that particular
        // instruction sequence to be used.

        // Instructions listed here specifically are considered always safe to generate. 
        switch (instr)
        {
        default:
            // Trigger a failure
        ...
        // Constant loading of an integer isn't always safe as the number may be a magic number from the runtime such as a size of a runtime data structure, or a runtime flag
        // LDIND/STIND isn't necessarily safe. The marshaller may have encoded the layout of a runtime data structure
        // Basic arithmetic isn't necessarily safe. Arithmetic may encode layouts/design of runtime
        // Logical arithmetic is also not necessarily safe. Again, runtime flags are interesting
        // Type system dependent operations such as isinst, box, and ldtoken are not necessarily safe, as the type isn't necessarily safely referencable
        // ldstr isn't safe as we have no means to push a string out through the R2R pathway.
        case CEE_NOP:
        case CEE_LDARG_0:
        case CEE_LDARG_1:
        case CEE_LDARG_2:
...
thread_local bool s_overrideNonR2RSafeILStubChecks = false;

void GenerateNonR2RSafeILStub()
{
    if (!s_overrideNonR2RSafeILStubChecks)
        ThrowHR(E_FAIL);
}
...
{
    s_overrideNonR2RSafeILStubChecks = oldR2RUnsafeValue;
}
