...
setlocal ENABLEDELAYEDEXPANSION
  ...
  rem set PERFHARNESS_PROJECT=%CORECLR_REPO%\tests\src\Common\PerfHarness\PerfHarness.csproj
  set PERFHARNESS_PROJECT=%CORECLR_REPO%\tests\src\Common\PerfHarness\project.json
  call :run_cmd "%CORECLR_REPO%\Tools\dotnetcli\dotnet.exe" restore "%PERFHARNESS_PROJECT%"                                        || (
    call :print_error Failed to restore "%PERFHARNESS_PROJECT%".
    exit /b 1
  )
  call :run_cmd "%CORECLR_REPO%\Tools\dotnetcli\dotnet.exe" publish "%PERFHARNESS_PROJECT%" -c Release -o "%CORECLR_REPO%\sandbox" || (
    call :print_error Failed to publish "%PERFHARNESS_PROJECT%".
    exit /b 1
  )
