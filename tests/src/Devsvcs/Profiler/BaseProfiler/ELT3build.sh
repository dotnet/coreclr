...
#[ -z "${Output:-}"       ] && Output=$PROFILER_TESTS_PATH/ELT3Profiler.so
Output=$PROFILER_TESTS_PATH/ELT3Profiler.so
...
CXX_FLAGS="$CXX_FLAGS --no-undefined -Wno-invalid-noreturn -fPIC -fms-extensions -DELT3 -DBIT64 -DPAL_STDCPP_COMPAT -DPLATFORM_UNIX -std=c++11"
...
clang++ -shared -o $Output $CXX_FLAGS $INCLUDES Common/ClassFactory.cpp Common/CorProfiler.cpp Common/dllmain.cpp Common/ProfilerCommon.cpp apitests/elt3.cpp
