...
{
    private static GCLatencyMode s_initialMode = GCSettings.LatencyMode;
    private static bool s_server = false;
    private static bool s_nonConcurrent = false;
    ...
    public static int Main()
    {
        ...
        InputValidationTest test = new InputValidationTest(s_server, s_nonConcurrent);

        return test.Run();
    }

    private static bool DetectInitialMode()
    {
        ...
        if (s_initialMode == GCLatencyMode.Batch)
        {
            s_nonConcurrent = true;
            Console.WriteLine("Non Concurrent ");
        }
        else if (s_initialMode == GCLatencyMode.Interactive)
        {
    ...
    private class InputValidationTest
    {
        ...
        {
            int errorCount = 0;
            Console.WriteLine("Initial mode is {0}", s_initialMode);
            for (int i = 0; i < validInputs.Count; i++)
            {
                ...
                GCSettings.LatencyMode = s_initialMode;
            }
            ...
            for (int i = 0; i < invalidInputs.Count; i++)
            {
                ...
                if (GCSettings.LatencyMode != s_initialMode)
                {
