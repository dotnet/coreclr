...
(for %%a in (%tests%) do (
    "%ProjectDir%Tools\dotnetcli\dotnet.exe" build --runtime win-x64 /p:Platform=x64 --configuration Checked "%ProjectDir%tests\src\readytorun\r2rdump\files\%%a\%%a.csproj"
    %ProjectDir%bin\tests\Windows_NT.x64.Checked\Tests\Core_Root\crossgen /readytorun /platform_assemblies_paths %ProjectDir%bin\tests\Windows_NT.x64.Checked\Tests\Core_Root /out %%a.ni.dll %ProjectDir%bin\tests\Windows_NT.x64.Checked\readytorun\r2rdump\R2RDumpTest\%%a.dll
...
(for %%a in (%tests%) do (
    "%ProjectDir%Tools\dotnetcli\dotnet.exe" build --runtime win-x86 /p:Platform=x86 --configuration Release "%ProjectDir%tests\src\readytorun\r2rdump\files\%%a\%%a.csproj"
    %ProjectDir%bin\tests\Windows_NT.x86.Release\Tests\Core_Root\crossgen /readytorun /platform_assemblies_paths %ProjectDir%bin\tests\Windows_NT.x86.Release\Tests\Core_Root /out %%a.ni.dll %ProjectDir%bin\tests\Windows_NT.x86.Release\readytorun\r2rdump\R2RDumpTest\%%a.dll
